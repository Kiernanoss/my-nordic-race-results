...
const dataProvider = {
  ...
  async getAthleteResults(athleteId) {
    const target = _athletes.find((a) => a.id === athleteId);
    const targetKey = target ? normalizeIdentity(`${target.firstName} ${target.lastName}`) : "";
    const rows = [];
    _races.forEach((r) => {
      const matches = (Array.isArray(r.results) ? r.results : []).filter((x) => {
        if (x?.athleteId === athleteId) return true;
        return targetKey && normalizeIdentity(`${x.firstName} ${x.lastName}`) === targetKey;
      });
...
function editDistance(a, b) {
  const aa = normalizeIdentity(a), bb = normalizeIdentity(b);
  if (aa === bb) return 0;
...
function athleteSimilarity(a, b) {
  const af = normalizeIdentity(a.firstName), bf = normalizeIdentity(b.firstName);
  const al = normalizeIdentity(a.lastName), bl = normalizeIdentity(b.lastName);
  if (!af || !bf || !al || !bl || al !== bl) return false;
  const sameTeam = normalizeIdentity(a.team) && normalizeIdentity(b.team) && normalizeIdentity(a.team) === normalizeIdentity(b.team);
  const sameSchool = normalizeIdentity(a.school) && normalizeIdentity(b.school) && normalizeIdentity(a.school) === normalizeIdentity(b.school);
  if (!sameTeam && !sameSchool) return false;
  const d = editDistance(af, bf);
  return af === bf || d <= (Math.min(af.length, bf.length) >= 6 ? 1 : 0);
}
function athleteCompleteness(r) {
  return (r.team ? 4 : 0) + (r.school ? 3 : 0) + (r.class || r.field ? 2 : 0) + (r.bib ? 1 : 0) + (Number.isFinite(Number(r.place)) ? 2 : 0) + (Number.isFinite(r.timeSec) ? 2 : 0);
}
function chooseBetterAthleteRow(a, b) {
  const sa = athleteCompleteness(a), sb = athleteCompleteness(b);
  if (sa !== sb) return sa > sb ? a : b;
  const pa = Number(a.place), pb = Number(b.place);
...
function parseScraperExport(text) {
  ...
  const races = [...byRace.values()].map((race) => {
    // Collapse duplicate athlete rows within a race. Endurance exports can
    // contain the same athlete twice, sometimes with one incomplete row.
    // Keep the richer/correct row so profile matching and Around Me use the
    // real result instead of a duplicate with a different place.
    const canonicalRows = [];
    race.rawRows.forEach((row) => {
      const idx = canonicalRows.findIndex((x) => {
        const sameName = normalizeIdentity(`${x.firstName} ${x.lastName}`) === normalizeIdentity(`${row.firstName} ${row.lastName}`);
        return sameName || athleteSimilarity(x, row);
      });
      if (idx < 0) canonicalRows.push(row);
      else canonicalRows[idx] = chooseBetterAthleteRow(canonicalRows[idx], row);
    });
    ...
    const rows = canonicalRows.slice();
...
export default function App() {
  ...
  const teamMembers = useMemo(() => {
    ...
    return combined.athletes.filter((a) => {
      ...
      return (team && team === wantedTeam) || (!team && school && school === wantedSchool) || (school && school === wantedTeam);
    });
