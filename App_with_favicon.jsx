...
function Dashboard({ appState, stats, myResults, goto, teamMembers, matches, myAthleteId, onDecide, onRefresh, refreshing }) {
  ...
  const chartData = [...myResults].reverse().map((r, i) => ({ i: i + 1, date: r.race.date, place: primaryPlace(r.result), name: r.race.raceName }));
        ...
        <>
          ...
          {latest && (
            <Card className="p-6">
              ...
              <div className="mb-4">
                <span className="nrr-display nrr-num text-3xl">{placeSuffix(primaryPlace(latest.result))}</span>
...
function RosterPage({ profile, teamMembers, myAthleteId, appState, setAppState, goto }) {
  ...
  const rows = teamMembers.map((a) => {
    ...
    const positions = results.map((r) => primaryPlace(r.result));
