...
const GlobalStyle = () => (
  <style>{`
    ...
    .nrr-root { font-family: 'Inter', sans-serif; color: var(--ink); background: var(--snow); font-size: 17px; }
    @media (min-width: 1024px) {
      .nrr-root { font-size: 18px; }
      .nrr-root .text-sm { font-size: 1rem; }
      .nrr-root .text-xs { font-size: 0.88rem; }
    }
    @media (max-width: 639px) {
      .nrr-root .text-sm { font-size: 1rem; }
      .nrr-root .text-xs { font-size: 0.9rem; }
      .nrr-root button { min-height: 42px; }
      .nrr-root input, .nrr-root select { min-height: 44px; }
    }
...
export default function App() {
  ...
  return (
    <div className="nrr-root min-h-screen flex">
      ...
      {/* Sidebar */}
      <aside className={`fixed sm:static z-30 inset-y-0 left-0 w-72 border-r flex flex-col transition-transform sm:translate-x-0 ${navOpen ? "translate-x-0" : "-translate-x-full"}`} style={{ borderColor: "#314A67", background: "#243B55", color: "#F4F8FC" }}>
        ...
        <>
            ...
            <div className="relative z-10 p-5 sm:p-6 flex flex-wrap items-end justify-between gap-4">
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-3 bg-white/80 rounded-xl px-4 py-3 border nrr-focus"
                  style={{ borderColor: "rgba(207,228,243,.9)" }}
                >
                    ...
                    <form onSubmit={saveProfileFromDropdown} className="grid grid-cols-2 gap-3">
                      {[["firstName","First name"],["lastName","Last name"],["preferredName","Preferred name"],["city","City"],["school","School"],["team","Team"]].map(([key,label]) => (
                        <label key={key} className="block">
                          ...
                          <input value={profileDraft?.[key] || ""} onChange={updateProfileDraft(key)} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm nrr-focus" style={{ borderColor: "var(--border)" }} />
                        </label>
                      ))}
                      <button type="submit" className="col-span-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-white nrr-focus" style={{ background: "var(--ice)" }}>
                        {profileSaved ? "Profile Saved ✓" : "Save Profile"}
                      </button>
                    </form>
                    <button type="button" onClick={() => { setProfileOpen(false); goto("settings"); }} className="mt-3 w-full rounded-lg px-3 py-2 text-sm font-medium border nrr-focus" style={{ borderColor: "var(--ice-line)", color: "var(--ice)" }}>
                      Open Full Settings
                    </button>
                  </div>
                )}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1.55fr_1fr] gap-4 sm:gap-5">
            <Card className="p-4 sm:p-6 min-w-0">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h3 className="font-semibold text-lg sm:text-xl">Performance Trend</h3>
            ...
            <Card className="p-4 sm:p-6 min-w-0">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-lg sm:text-xl">Recent Races</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5">
            <Card className="p-4 sm:p-6">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-lg sm:text-xl">Team Roster</h3>
            ...
            <Card className="p-4 sm:p-6">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-lg sm:text-xl">Season Summary</h3>
...
function RosterPage({ profile, teamMembers, myAthleteId, appState, setAppState, goto }) {
  ...
  // Every athlete is classified from their own most recent race.
  // They appear in exactly one section, and each section is always shown.
  // Within a section, fastest most-recent-race time comes first.
  const rosterGroups = ROSTER_CLASS_ORDER.map((classKey) => {
    const rows = athleteRows
      .filter((row) => row.latest && row.latestClass === classKey)
      .sort((a, b) => {
        const ta = Number(a.latest.result.totalTimeSec);
        const tb = Number(b.latest.result.totalTimeSec);
    ...
    return { classKey, rows: rows.map((row, index) => ({ ...row, teamPlace: index + 1 })) };
  });

  const labels = { GVAR: "Girls Varsity", BVAR: "Boys Varsity", GJV: "Girls JV", BJV: "Boys JV" };
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <p className="text-sm" style={{ color: "var(--slate)" }}>Each racer is placed by their own most recent race and sorted fastest to slowest by that race time.</p>
      </div>
      {rosterGroups.map((group) => (
        <Card key={group.classKey} className="p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="nrr-display text-xl">{labels[group.classKey]}</h2>
            <span className="text-xs" style={{ color: "var(--slate)" }}>{group.rows.length} athlete{group.rows.length === 1 ? "" : "s"}</span>
          </div>
          {group.rows.length === 0 ? (
            <div className="text-sm py-3" style={{ color: "var(--slate)" }}>No athletes in this group.</div>
          ) : (
            <div className="flex flex-col divide-y" style={{ borderColor: "var(--border-soft)" }}>
              {group.rows.map((r) => (
                <div key={`${group.classKey}-${r.athlete.id}`} className="flex items-center justify-between gap-4 py-3">
                  ...
                  <div className="flex items-center gap-5 shrink-0">
                    <div className="text-right">
                      ...
                      <div className="text-xs" style={{ color: "var(--slate)" }}>Roster place</div>
                    </div>
