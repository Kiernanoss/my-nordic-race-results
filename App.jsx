...
function parseScraperExport(text) {
  ...
  matches.forEach((m) => {
    ...
    rows.forEach((r) => {
      ...
      const dedupeKey = `${norm.firstName}|${norm.lastName}|${norm.field}|${norm.place}|${norm.time}|${norm.bib}`;
...
/** Assigns stable athlete ids to imported rows and collapses obvious
 * spelling variants for the same teammate. The scraper remains untouched.
 * A merge requires the same last name, the same team/school, and either the
 * same first name or a one-edit first-name typo (e.g. Torsten/Torsen).
 */
function firstNameDistanceAtMostOne(a, b) {
...
function sameAthleteGroup(a, b) {
  const firstA = normalizeIdentity(a.firstName);
  const firstB = normalizeIdentity(b.firstName);
  const lastA = normalizeIdentity(a.lastName);
  const lastB = normalizeIdentity(b.lastName);
  if (!firstA || !firstB || !lastA || lastA !== lastB) return false;

  const teamA = normalizeIdentity(a.team);
  const teamB = normalizeIdentity(b.team);
  const schoolA = normalizeIdentity(a.school);
  const schoolB = normalizeIdentity(b.school);
  const sameTeam = !!teamA && !!teamB && teamA === teamB;
  const sameSchool = !!schoolA && !!schoolB && schoolA === schoolB;
  const exactName = firstA === firstB;

  // Exact same name: allow an incomplete source row to merge with a richer
  // row, but only when the race evidence also lines up. This handles cases
  // where Endurance gives the same athlete once with team data and once with
  // blank team/school data.
  if (exactName) {
    if (sameTeam || sameSchool) return true;
    const oneSideMissingIdentity = (!teamA && !schoolA) || (!teamB && !schoolB);
    if (!oneSideMissingIdentity) return false;
    const sameField = normalizeIdentity(a.field || a.class) === normalizeIdentity(b.field || b.class);
    const sameBib = a.bib && b.bib && String(a.bib) === String(b.bib);
    const sameTime = a.timeSec != null && b.timeSec != null && Math.abs(Number(a.timeSec) - Number(b.timeSec)) < 0.2;
    return sameField && (sameBib || sameTime);
  }

  // A spelling variant is merged only when team/school agrees, so two
  // different athletes with similar names are not accidentally combined.
  if (!sameTeam && !sameSchool) return false;
  return firstNameDistanceAtMostOne(firstA, firstB);
}

function buildCombinedDataset(importedRaces) {
  const sourceRaces = Array.isArray(importedRaces) ? importedRaces : [];
  const groups = [];
  const races = sourceRaces.map((race) => ({ ...race, results: (race.results || []).map((r) => ({ ...r })) }));

  // Build one canonical athlete record across all races.
  races.forEach((race) => race.results.forEach((r) => {
    const existing = groups.find((g) => sameAthleteGroup(g.representative, r));
    if (existing) {
      existing.rows.push(r);
      existing.nameCounts[`${r.firstName || ""} ${r.lastName || ""}`] = (existing.nameCounts[`${r.firstName || ""} ${r.lastName || ""}`] || 0) + 1;
      if (!existing.representative.team && r.team) existing.representative.team = r.team;
      if (!existing.representative.school && r.school) existing.representative.school = r.school;
      if (!existing.representative.city && r.city) existing.representative.city = r.city;
    } else {
      groups.push({
        representative: { ...r },
        rows: [r],
        nameCounts: { [`${r.firstName || ""} ${r.lastName || ""}`]: 1 },
      });
    }
  }));
  ...
  groups.forEach((g, index) => {
    const counts = Object.entries(g.nameCounts).sort((a, b) => b[1] - a[1] || a[0].length - b[0].length);
    const displayName = counts[0]?.[0] || `${g.representative.firstName || ""} ${g.representative.lastName || ""}`;
    const parts = displayName.trim().split(/\s+/);
    const firstName = parts.slice(0, -1).join(" ") || g.representative.firstName || "";
    const lastName = parts.length > 1 ? parts[parts.length - 1] : (g.representative.lastName || "");
    const team = g.representative.team || "";
    const school = g.representative.school || "";
    const id = `imp_ath_${stableHash(`person:${normalizeIdentity(firstName)} ${normalizeIdentity(lastName)}|team:${normalizeIdentity(team)}|school:${normalizeIdentity(school)}`)}`;
    g.id = id;
    g.display = { ...g.representative, id, firstName, lastName, team, school };
    g.rows.forEach((r) => groupByRow.set(r, g));
  });

  // Point every result at the canonical athlete and remove duplicate copies
  // of that athlete within the same race/field/distance. Prefer the richer
  // row, then the better source place when two copies remain.
  races.forEach((race) => {
    const seen = new Map();
    const deduped = [];
    race.results.forEach((r) => {
      const g = groupByRow.get(r);
      const athleteId = g?.id || r.athleteId || `imp_ath_${stableHash(athleteIdentityKey(r))}`;
      r.athleteId = athleteId;
      if (g?.display) {
        r.firstName = g.display.firstName;
        r.lastName = g.display.lastName;
        if (!r.team) r.team = g.display.team;
        if (!r.school) r.school = g.display.school;
      }
      const key = `${athleteId}|${normalizeIdentity(r.class || r.field || "")}|${r.distanceKm ?? ""}`;
      ...
      const score = (x) => (x.team ? 4 : 0) + (x.school ? 3 : 0) + (x.bib && x.bib !== "Not available" ? 1 : 0) + (Number.isFinite(x.totalTimeSec) ? 1 : 0) + (Number.isFinite(x.overallPosition) ? 1 : 0);
      const pPlace = Number(previous.overallPosition);
      const rPlace = Number(r.overallPosition);
      if (score(r) > score(previous) || (score(r) === score(previous) && Number.isFinite(rPlace) && (!Number.isFinite(pPlace) || rPlace < pPlace))) {
        deduped[previousIndex] = r;
      }
    });
    race.results = deduped;
  });

  const registry = new Map();
  groups.forEach((g) => {
    registry.set(g.id, g.display);
  });
...
export default function App() {
  ...
  const stats = useMemo(() => {
    ...
    const positions = myResults.map((r) => r.result.overallPosition).filter(Number.isFinite);
      ...
      {!stats ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-6 gap-4">
            ...
            <StatCard label="Best 5K" value={stats.best5k} sub={stats.best5kPace} icon={TrendingUp} /><StatCard label="Best 2.5K" value={stats.best2_5k} sub={stats.best2_5kPace} icon={TrendingUp} />
...
function RosterPage({ profile, teamMembers, myAthleteId, appState, setAppState, goto }) {
  ...
  const rows = teamMembers.map((a) => {
    ...
    return {
      ...
      bestTime: (() => { const r = results.filter((x) => Number.isFinite(x.result.totalTimeSec)).sort((a, b) => (a.result.distanceKm || 999) - (b.result.distanceKm || 999) || a.result.totalTimeSec - b.result.totalTimeSec)[0]; return r ? `${r.result.totalTime} (${distanceLabel(r.result.distanceKm)})` : "Not available"; })(),
...
function PerformancePage({ myResults, goto }) {
  ...
  const chartData = [...myResults].reverse().map((r, i) => ({ i: i + 1, place: r.result.overallPosition, pace: pacePerKm(r.result.totalTimeSec, r.result.distanceKm), distance: r.result.distanceKm, name: r.race.raceName }));
