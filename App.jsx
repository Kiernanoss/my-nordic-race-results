...
function rosterClassKey(value) {
  const key = normalizeIdentity(value).replace(/\s+/g, "").toUpperCase();
...
function RosterPage({ profile, teamMembers, myAthleteId, appState, setAppState, goto }) {
  ...
  const athleteRows = teamMembers.map((a) => {
    ...
    const latestByClass = {};
    results.forEach((r) => {
      const key = rosterClassKey(r.result.class);
      if (!latestByClass[key] || new Date(r.race.date) > new Date(latestByClass[key].race.date)) {
        latestByClass[key] = r;
      }
    });
    return { athlete: a, results, latestByClass };
  });

  const rosterGroups = ROSTER_CLASS_ORDER.map((classKey) => {
    const rows = athleteRows
      .map((row) => ({ ...row, latest: row.latestByClass[classKey] }))
      .filter((row) => row.latest);

    // Team place is calculated from each athlete's most recent race in this
    // class, using class placement and only athletes from the same team.
    rows.sort((a, b) => {
  ...
  const otherRosterRows = athleteRows
    .map((row) => ({ ...row, latest: row.results.slice().sort((a, b) => new Date(b.race.date) - new Date(a.race.date))[0] }))
    .filter((row) => row.latest && !ROSTER_CLASS_ORDER.includes(rosterClassKey(row.latest.result.class)))
    .sort((a, b) => new Date(b.latest.race.date) - new Date(a.latest.race.date));
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {otherRosterRows.length > 0 && (
        <Card className="p-5">
          <h2 className="nrr-display text-xl mb-4">Other Classes</h2>
          <div className="flex flex-col divide-y" style={{ borderColor: "var(--border-soft)" }}>
            {otherRosterRows.map((r) => (
              <div key={`other-${r.athlete.id}`} className="flex items-center justify-between gap-4 py-3">
                <button onClick={() => goto("athlete", r.athlete.id)} className="nrr-focus text-left">
                  <div className="font-medium">{r.athlete.firstName} {r.athlete.lastName}{r.athlete.id === myAthleteId ? " (me)" : ""}</div>
                  <div className="text-xs" style={{ color: "var(--slate)" }}>{r.latest.result.class} · {r.latest.race.raceName}</div>
                </button>
                <span className="nrr-num">{placeSuffix(primaryPlace(r.latest.result))}</span>
              </div>
            ))}
          </div>
        </Card>
