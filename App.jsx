          ...
          <div className="relative overflow-visible rounded-2xl border" style={{ borderColor: "var(--ice-line)", background: "linear-gradient(110deg,#F4FAFE 0%,#EAF4FB 55%,#DCEEF9 100%)" }}>
            ...
            <div className="relative z-10 p-5 sm:p-6 flex flex-wrap items-end justify-between gap-4">
              ...
              <div className="relative">
                ...
                {profileOpen && (
                  <div className="absolute right-0 top-full mt-2 z-30 w-72 bg-white rounded-xl border shadow-lg p-4" style={{ borderColor: "var(--ice-line)" }}>
                    <div className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: "var(--ice)" }}>Athlete Profile</div>
                    <div className="space-y-2 text-sm">
                      <div><span style={{ color: "var(--slate)" }}>Name:</span> <span className="font-medium">{p.firstName} {p.lastName}</span></div>
                      <div><span style={{ color: "var(--slate)" }}>City:</span> <span className="font-medium">{p.city || "—"}</span></div>
                      <div><span style={{ color: "var(--slate)" }}>School:</span> <span className="font-medium">{p.school || "—"}</span></div>
                      <div><span style={{ color: "var(--slate)" }}>Team:</span> <span className="font-medium">{p.team || "—"}</span></div>
                    </div>
                    <button type="button" onClick={() => { setProfileOpen(false); goto("settings"); }} className="mt-4 w-full rounded-lg px-3 py-2 text-sm font-medium border nrr-focus" style={{ borderColor: "var(--ice-line)", color: "var(--ice)" }}>
                      Edit Profile
                    </button>
                  </div>
                )}
