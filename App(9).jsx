...
export default function App() {
  ...
  const teamSeasons = useMemo(() => {
    if (!appState.profile) return [];
    const wanted = normalizeIdentity(appState.profile.team || appState.profile.school);
    if (!wanted) return [];
    const seasons = new Set();
    combined.races.forEach((race) => {
      race.results.forEach((result) => {
        const team = normalizeIdentity(result.team || result.school);
        if (team && (team === wanted || normalizeIdentity(result.school) === wanted)) {
    ...
    <div className="nrr-root min-h-screen flex">
      ...
      <main className="flex-1 min-w-0 pb-20 sm:pb-0">
        ...
        <div className="max-w-5xl mx-auto p-5 sm:p-8">
          ...
          {page === "roster" && <RosterPage profile={appState.profile} teamMembers={teamMembers} teamSeasons={teamSeasons} myAthleteId={myAthleteId} appState={appState} setAppState={setAppState} goto={goto} />}
...
function SeasonsPage({ myResults, seasons, goto }) {
  ...
  useEffect(() => {
    if (!selectedSeason || !seasons.includes(selectedSeason)) setSelectedSeason(latestSeason);
  }, [seasons, latestSeason, selectedSeason]);

  const rows = selectedSeason ? myResults.filter((r) => r.race.season === selectedSeason) : [];
  const positions = rows.map((r) => primaryPlace(r.result)).filter(Number.isFinite);
  const best5k = rows.filter((r) => r.result.distanceKm === 5 && Number.isFinite(r.result.totalTimeSec)).sort((a, b) => a.result.totalTimeSec - b.result.totalTimeSec)[0];
  const best2_5k = rows.filter((r) => r.result.distanceKm === 2.5 && Number.isFinite(r.result.totalTimeSec)).sort((a, b) => a.result.totalTimeSec - b.result.totalTimeSec)[0];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {seasons.length > 0 && (
          <select value={selectedSeason || ""} onChange={(e) => setSelectedSeason(e.target.value)} className="nrr-focus text-sm border rounded-full px-3 py-1.5" style={{ borderColor: "var(--border)" }}>
            {seasons.map((season) => <option key={season}>{season}</option>)}
          </select>
        )}
      </div>
      {!selectedSeason ? (
        <Card className="p-6 text-sm" style={{ color: "var(--slate)" }}>No seasons found yet.</Card>
      ) : rows.length === 0 ? (
        <Card className="p-6 text-sm" style={{ color: "var(--slate)" }}>No races for this season yet.</Card>
      ) : (
        <Card className="p-6">
...
/* ============================================================================
   ROSTER PAGE
============================================================================ */
function RosterPage({ profile, teamMembers, teamSeasons, myAthleteId, appState, setAppState, goto }) {
  const [rowsByAthlete, setRowsByAthlete] = useState({});
  const latestSeason = teamSeasons.length ? teamSeasons[teamSeasons.length - 1] : null;
  const [selectedSeason, setSelectedSeason] = useState(latestSeason);
  ...
  useEffect(() => {
    if (!selectedSeason || !teamSeasons.includes(selectedSeason)) setSelectedSeason(latestSeason);
  }, [teamSeasons, latestSeason, selectedSeason]);

  const athleteRows = teamMembers.map((a) => {
    const results = (rowsByAthlete[a.id] || [])
      .filter((row) => !selectedSeason || row.race.season === selectedSeason)
      .slice()
      .sort((a, b) => {
    ...
    return { athlete: a, results, latest, latestClass: latest ? rosterClassKey(latest.result.class, latest.race.raceName) : null };
  }).filter((row) => row.latest);

  const rosterGroups = ROSTER_CLASS_ORDER.map((classKey) => {
    const rows = athleteRows.filter((row) => row.latestClass === classKey);
    rows.sort((a, b) => {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm" style={{ color: "var(--slate)" }}>Athletes are included once they race in the selected season.</p>
        </div>
        {teamSeasons.length > 0 && (
          <select value={selectedSeason || ""} onChange={(e) => setSelectedSeason(e.target.value)} className="nrr-focus text-sm border rounded-full px-3 py-1.5" style={{ borderColor: "var(--border)" }}>
            {teamSeasons.map((season) => <option key={season}>{season}</option>)}
          </select>
        )}
      </div>
      {!selectedSeason ? (
        <Card className="p-6 text-sm" style={{ color: "var(--slate)" }}>No team seasons found yet.</Card>
      ) : rosterGroups.length === 0 ? (
        <Card className="p-6 text-sm" style={{ color: "var(--slate)" }}>No athletes have raced for this team in {selectedSeason} yet.</Card>
...
function PerformancePage({ myResults, goto }) {
  const seasons = [...new Set(myResults.map((r) => r.race.season).filter(Boolean))].sort();
  const latestSeason = seasons.length ? seasons[seasons.length - 1] : null;
  const [selectedSeason, setSelectedSeason] = useState(latestSeason || "All Time");

  useEffect(() => {
    if (selectedSeason !== "All Time" && !seasons.includes(selectedSeason)) setSelectedSeason(latestSeason || "All Time");
  }, [seasons, latestSeason, selectedSeason]);

  const visibleResults = selectedSeason === "All Time"
    ? myResults.slice().sort((a, b) => new Date(a.race.date) - new Date(b.race.date))
    : myResults.filter((r) => r.race.season === selectedSeason).slice().sort((a, b) => new Date(a.race.date) - new Date(b.race.date));
  const chartData = visibleResults.map((r, i) => ({ i: i + 1, place: primaryPlace(r.result), pace: pacePerKm(r.result.totalTimeSec, r.result.distanceKm), distance: r.result.distanceKm, name: r.race.raceName, date: r.race.date }));
