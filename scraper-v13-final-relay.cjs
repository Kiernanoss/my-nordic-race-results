...
function parseRelayFinalPdfText(text, genderHint = null, fieldHint = null) {
  ...
  // Final-result PDFs use a different table from the semifinal PDFs:
  //
  // Tm Final POS Bib FIRST NAME LAST NAME TEAM NAME DIV RELAY_PTS SEX
  // 19:32.7 3 261/264 Mason Mahmood & Liam MacChesney Mahtomedi Boys Final  ...
  //
  // Detect the layout from its header rather than from a race/event ID.
  for (const line of lines.slice(0, 12)) {
    if (/\bFINAL\s+POS\b/i.test(line) && /\bRELAY[_\s]*PTS\b/i.test(line) && /\bSEX\b/i.test(line)) {
      finalLayout = true;
      break;
    }
  }
  if (!finalLayout) return out;
  ...
  function addFinalRow(place, bib, firstPartner, secondPartner, team, time, sex, div) {
    ...
    const inferredGender = sex
      ...
      : genderHint;
    const inferredField = div || fieldHint || '';
  ...
  function parseLine(line) {
    // Optional finish time is present for competitors that have a recorded
    // time. Some later/non-scoring rows omit it, so it must be optional.
    let rest = line;
    let time = '';
    ...
    // Sex is the last column in the final layout.
    const sexM = body.match(/\s+([MF])$/i);
    if (!sexM) return false;
    const sex = sexM[1].toUpperCase();
    body = body.slice(0, sexM.index).trim();

    // Relay points are immediately before the sex column.
    const ptsM = body.match(/\s+(\d{2,3})$/);
    if (!ptsM || !pointsRe.test(ptsM[1])) return false;
    body = body.slice(0, ptsM.index).trim();

    // DIV is immediately before relay points. Support both the short relay
    // divisions used by section championships and the explicit Boys/Girls
    // Final wording used by team-relay PDFs.
    let div = '';
    ...
    if (!firstPartner || !remainder) return false;

    // In these PDFs the two athlete names occupy the FIRST NAME/LAST NAME
    // columns and the club/team begins after the second athlete. Use the
    // standard two-token athlete form first, then allow a three-token second
    // name when the remainder would otherwise be empty or implausibly short.
    ...
    // If there is no team after a two-token second name, keep the entire
    // remainder as the partner; this also safely rejects malformed rows.
    if (!team) {
      if (tokens.length >= 3) {
    ...
    addFinalRow(place, bib, firstPartner, secondPartner, team, time, sex, div);
    return true;
  }
