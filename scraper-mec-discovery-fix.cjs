...
async function processRace(race, a, stats, browser) {
  ...
  const findFieldSelect = async () => {
    ...
    return selects.find(s => {
      ...
      const hasPlaceholder = texts.includes("select a field");
      const hasCoreFields = ["bjv", "bvar", "gjv", "gvar"].every(f => texts.includes(f));
      return hasPlaceholder || hasCoreFields;
    }) || null;
  };
  ...
  try {
    ...
    if (initialFieldSelect) {
      const fields = initialFieldSelect.options
        .map(o => o.text.trim())
        .filter(t => t && !/^select\s+a\s+field$/i.test(t));

      console.log(`  FIELD FILTER: scraping ${fields.length} source fields individually: ${fields.join(', ')}`);
      ...
      for (const fieldText of fields) {
        ...
        if (!fieldSelect) {
          console.warn(`  FIELD FILTER: field selector disappeared before ${fieldText}; stopping field loop.`);
          break;
        }

        await page.locator('select').nth(fieldSelect.index).selectOption({ label: fieldText });
        await waitForPostback();
        ...
        if (sizeSelect) {
          try {
            ...
            console.warn(`  GRID: could not set page size to 50 for ${fieldText}: ${e.message}`);
          }
        }

        console.log(`  FIELD FILTER: ${fieldText}`);
