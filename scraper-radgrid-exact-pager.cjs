  ...
  const scrapeAllGridPages = async () => {
    ...
    const findPagerControl = async (wantedPage) => {
      return page.evaluate((wanted) => {
        ...
        // Endurance Promotions uses a Telerik RadGrid pager. The page-number
        // anchors are rendered inside .rgNumPart and contain the exact
        // javascript: doPostBack(...) call used by the live site.
        const pager = grid.querySelector('.rgNumPart');
        if (!pager) return null;

        const anchor = [...pager.querySelectorAll('a')].find((el) => {
          const text = (el.textContent || '').trim();
          return text === String(wanted) && !el.classList.contains('rgCurrentPage');
        });
        ...
        return {
          ...
          pageText: (anchor.textContent || '').trim()
        };
      }, wantedPage);
    };

    const getCurrentPagerPage = async () => page.evaluate(() => {
      const grid = document.querySelector('#ctl00_cphMain_grdIndividualResults');
      if (!grid) return null;
    ...
    const getMainGridSignature = async () => page.evaluate(() => {
      const grid = document.querySelector('#ctl00_cphMain_grdIndividualResults');
      if (!grid) return '';
    ...
    const triggerNextPage = async (nextPage, previousGridSignature) => {
      const control = await findPagerControl(nextPage);

      if (!control) {
        console.warn(`  GRID: could not locate page ${nextPage} inside the Individual Results RadGrid pager; stopping.`);
        return false;
      }

      console.log(`  GRID: exact RadGrid page ${nextPage} link: ${control.outerHTML}`);

      const href = String(control.href || '').trim();
      if (!href.toLowerCase().startsWith('javascript:')) {
        console.warn(`  GRID: page ${nextPage} link is not a javascript postback; stopping.`);
        return false;
      }

      // IMPORTANT: the live site uses `doPostBack(...)`, not necessarily
      // `__doPostBack(...)`, and the argument is often empty. Execute the
      // exact javascript payload emitted by the Telerik RadGrid rather than
      // inventing Page$2/Page$3 arguments.
      const js = href.replace(/^javascript:\s*/i, '');
      await page.evaluate((code) => {
        // The code comes directly from the page's own RadGrid pager anchor.
        // Execute it in the page context exactly as the browser would when
        // the user clicks the link.
        (0, eval)(code);
      }, js);

      // Telerik commonly updates this grid through an asynchronous postback,
      // so networkidle/load events alone are not reliable. Wait for the RadGrid
      // itself to mark the requested page as current.
      let activePage = null;
      try {
      ...
      activePage = await getCurrentPagerPage();
      const afterSignature = await getMainGridSignature();
      console.log(`  GRID: after exact RadGrid postback, current page = ${activePage || 'unknown'}`);

      if (activePage === String(nextPage)) return true;
      if (afterSignature && afterSignature !== previousGridSignature) {
        console.log(`  GRID: result grid changed even though the current-page marker was not detected.`);
        return true;
      }

      // Give Telerik one more brief window to finish its async update.
      await page.waitForTimeout(750);
      activePage = await getCurrentPagerPage();
      const retrySignature = await getMainGridSignature();
      console.log(`  GRID: retry check after wait, current page = ${activePage || 'unknown'}`);

      return activePage === String(nextPage) || (retrySignature && retrySignature !== previousGridSignature);
    };
    ...
    while (pageNo <= totalPages && pageNo <= 200) {
      ...
      const moved = await triggerNextPage(nextPage, previousSignature);
      if (!moved) break;
