...
async function processRace(race, a, stats, browser) {
  ...
  const scrapeAllGridPages = async () => {
    ...
    const triggerNextPage = async (nextPage) => {
      const controls = await findPagerControls(nextPage);

      if (!controls.length) {
        console.warn(`  GRID: could not locate any live pager control for page ${nextPage}; stopping.`);
        return false;
      }

      const beforeSignature = await getMainGridSignature();
      console.log(`  GRID: found ${controls.length} candidate pager control(s) for page ${nextPage}`);

      const submitFormPager = async (control) => {
        return page.evaluate(({ wanted, outerHTML }) => {
          ...
          const fromMarkup = String(outerHTML || '');
          const post = parsePostback(fromMarkup);
          const form = document.querySelector('form');
          if (!form) return { ok: false, reason: 'no-form' };

          let target = post?.target || '';
          let argument = post?.argument || '';

          // Some versions of the grid render the pager as a normal submit
          // control rather than an explicit __doPostBack link. Preserve the
          // submit control's name/value when possible.
          const name = fromMarkup.match(/\bname\s*=\s*["']([^"']+)["']/i)?.[1] || '';
          const value = fromMarkup.match(/\bvalue\s*=\s*["']([^"']*)["']/i)?.[1] || '';
          const onclick = fromMarkup.match(/\bonclick\s*=\s*["']([^"']*)["']/i)?.[1] || '';
          const onclickPost = parsePostback(onclick);
          if (!target && onclickPost) {
            target = onclickPost.target;
            argument = onclickPost.argument;
          }

          // Fall back to the known Individual Results GridView event target.
          // This is only used when the rendered control does not expose the
          // target explicitly.
          if (!target) target = 'ctl00$cphMain$grdIndividualResults';
          if (!argument) argument = `Page$${wanted}`;

          const setHidden = (fieldName, fieldValue) => {
            let input = form.elements[fieldName];
            if (input && input.length && !input.value) input = input[0];
          ...
          setHidden('ctl00_cphMain_grdIndividualResultsChangePage', `${wanted}_50`);

          // Preserve a native submit button's name/value if the pager was
          // rendered as one. This supplements, rather than replaces, the
          // ASP.NET hidden state already present in the form.
          if (name && value && !/^__/.test(name) && !name.includes('ChangePage')) {
            setHidden(name, value);
          }

          form.submit();
          return { ok: true, target, argument, name, value };
        }, { wanted: nextPage, outerHTML: control.outerHTML || '' });
      };

      for (let i = 0; i < controls.length; i++) {
        const control = controls[i];
        console.log(`  GRID: trying pager candidate ${i + 1}/${controls.length} (${control.mode}${control.target ? ` target=${control.target}` : ''})`);
        ...
        try {
          if (control.mode === 'postback') {
            ...
            // First perform the normal browser click, exactly as the known
            // working scraper does.
          ...
          for (let attempt = 0; attempt < 6; attempt++) {
            await page.waitForTimeout(250);
            const afterSignature = await getMainGridSignature();
            if (afterSignature && afterSignature !== beforeSignature) {
              console.log(`  GRID: pager candidate ${i + 1} changed the main result grid.`);
              return true;
            }
          }

          // Critical fallback: if the live click/postback left the grid on
          // the same rows, submit the CURRENT ASP.NET form state ourselves.
          // Unlike the earlier URL experiment, this keeps __VIEWSTATE,
          // __EVENTVALIDATION, selected values, cookies, and other hidden
          // inputs exactly as the browser currently has them.
          console.warn(`  GRID: live pager did not change the grid; submitting the current ASP.NET form state for page ${nextPage}.`);
          const submitted = await submitFormPager(control);
          if (!submitted.ok) {
            console.warn(`  GRID: form-state submission unavailable (${submitted.reason || 'unknown reason'}).`);
            continue;
          }
          ...
          for (let attempt = 0; attempt < 10; attempt++) {
            ...
            const afterSignature = await getMainGridSignature();
            if (afterSignature && afterSignature !== beforeSignature) {
              console.log(`  GRID: form-state page ${nextPage} submission changed the main result grid.`);
              return true;
            }
          }

          console.warn(`  GRID: form-state submission also left the main result grid unchanged.`);
        } catch (err) {
          console.warn(`  GRID: pager candidate ${i + 1} failed: ${err.message}`);
        }
      }

      console.warn(`  GRID: none of the page ${nextPage} pager methods changed the main result grid.`);
      return false;
    };
