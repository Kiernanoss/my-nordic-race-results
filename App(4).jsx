...
/** Small edit-distance helper used only for matching likely spelling variants.
 * It never merges athletes unless their last name AND team/school also match.
 */
...
function athleteRowCompleteness(r) {
  return (r.team ? 5 : 0) + (r.school ? 4 : 0) + (r.class ? 2 : 0) +
    (r.bib && r.bib !== "Not available" ? 1 : 0) +
    (Number.isFinite(r.overallPosition) ? 1 : 0) +
    (Number.isFinite(r.totalTimeSec) ? 1 : 0);
}

/** Assigns stable athlete ids to imported rows and collapses harmless source
 * spelling variants (for example a one-letter first-name typo) when the
 * athlete's last name and team/school agree. The displayed spelling is the
 * spelling that occurs most often in the imported results.
 */
function buildCombinedDataset(importedRaces) {
  const races = Array.isArray(importedRaces) ? importedRaces.map((race) => {
    // First remove duplicate copies of the same named athlete inside one race.
    // Prefer the richer row so a blank-team duplicate cannot win.
    const byName = new Map();
    (race.results || []).forEach((r) => {
      const nameKey = normalizeIdentity(`${r.firstName || ""} ${r.lastName || ""}`);
      const old = byName.get(nameKey);
      if (!old || athleteRowCompleteness(r) > athleteRowCompleteness(old)) byName.set(nameKey, r);
    });
    return { ...race, results: [...byName.values()].map((r) => ({ ...r })) };
  }) : [];

  // Start with exact identities.
  const registry = new Map();
  const resultRecords = [];
  races.forEach((race) => race.results.forEach((r) => {
    const id = r.athleteId || `imp_ath_${stableHash(athleteIdentityKey(r))}`;
    const record = {
      id,
    ...
    resultRecords.push({ race, result: r, id });
  ...
  // Merge only very close first-name spellings when last name and team/school
  // match. This handles source typos without merging unrelated teammates.
  ...
  entries.forEach((a) => {
    ...
    for (const c of canonical.values()) {
      const sameLast = normalizeIdentity(a.lastName) === normalizeIdentity(c.lastName);
      const sameTeam = a.team && c.team && normalizeIdentity(a.team) === normalizeIdentity(c.team);
      const sameSchool = a.school && c.school && normalizeIdentity(a.school) === normalizeIdentity(c.school);
      const firstDistance = nameEditDistance(a.firstName, c.firstName);
      if (sameLast && (sameTeam || sameSchool) && firstDistance <= 1 && firstDistance > 0) {
        chosen = c;
        break;
      }
    }
    canonical.set(a.id, chosen.id);
  });

  // Choose the most common spelling for each canonical athlete.
  const canonicalGroups = new Map();
  entries.forEach((a) => {
    const cid = canonical.get(a.id) || a.id;
    if (!canonicalGroups.has(cid)) canonicalGroups.set(cid, []);
    canonicalGroups.get(cid).push(a);
  });
  const finalAthletes = new Map();
  canonicalGroups.forEach((group, cid) => {
    group.sort((a, b) => b.count - a.count || b.firstName.length - a.firstName.length);
    const winner = group[0];
    finalAthletes.set(cid, { ...winner, id: cid });
  });

  // Rewrite every result to its canonical athlete id.
  resultRecords.forEach(({ result, id }) => {
    result.athleteId = canonical.get(id) || id;
  });

  return { races, athletes: [...finalAthletes.values()] };
}
