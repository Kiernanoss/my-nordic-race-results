...
async function processRace(race, a, stats, browser) {
  ...
  const scrapeAllGridPages = async () => {
    ...
    const triggerNextPage = async (nextPage) => {
      // Endurance Promotions uses a Telerik RadGrid. The page-number anchor
      // inside the Individual Results grid contains the exact javascript
      // postback target, but evaluating that javascript ourselves can break
      // ASP.NET's PageRequestManager in Playwright's strict-mode context.
      // Instead, click the exact live anchor with Playwright so the browser
      // executes the site's own handler exactly as a user click would.
      const pagerLinks = page.locator(
        '#ctl00_cphMain_grdIndividualResults .rgNumPart a'
      );

      const matches = await pagerLinks.evaluateAll((links, wanted) => {
        return links
          .map((el, index) => ({
            index,
          ...
          .filter(x => x.text === String(wanted));
      }, nextPage);

      if (!matches.length) {
        console.warn(
          `  GRID: could not locate page ${nextPage} inside the Individual Results RadGrid pager; stopping.`
        );
        return false;
      }
      ...
      console.log(
        `  GRID: exact RadGrid page ${nextPage} link: ${match.html}`
      );

      const beforeSignature = await page.evaluate(() => {
        const grid = document.querySelector(
          '#ctl00_cphMain_grdIndividualResults'
        );
        if (!grid) return '';
      ...
      const target = pagerLinks.nth(match.index);
      await target.scrollIntoViewIfNeeded();

      // Do not execute the href ourselves. A real Playwright click lets the
      // site's Telerik/ASP.NET JavaScript and Microsoft AJAX infrastructure
      // handle the postback in its own context.
      await target.click({ noWaitAfter: true });

      // Wait for Telerik to mark the requested page as current. This is more
      // reliable than networkidle because the grid normally updates through
      // an asynchronous partial postback.
      let activePage = null;
      try {
        await page.waitForFunction((wanted) => {
          ...
          return current &&
            (current.textContent || '').trim() === String(wanted);
        }, nextPage, { timeout: Math.min(a.timeout, 12000) });
      } catch {}
      ...
      console.log(
        `  GRID: exact RadGrid click result -> current page ${activePage || 'unknown'}`
      );

      if (activePage === String(nextPage)) return true;
      if (afterSignature && afterSignature !== beforeSignature) {
        console.log(
          `  GRID: result grid changed even though the current-page marker was not detected.`
        );
        return true;
      }

      // Give the AJAX partial postback a little additional time before
      // declaring failure.
      ...
      const retrySignature = await page.evaluate(() => {
        const grid = document.querySelector(
          '#ctl00_cphMain_grdIndividualResults'
        );
        if (!grid) return '';
      ...
      console.log(
        `  GRID: exact RadGrid retry check -> current page ${activePage || 'unknown'}`
      );

      return activePage === String(nextPage) ||
        Boolean(retrySignature && retrySignature !== beforeSignature);
    };
