...
// Endurance occasionally has a one-letter spelling error in an athlete's
// name (for example, "Torsen" vs "Torsten"). Treat very-close first-name
// spellings as the same athlete only when the last name and team/school also
// agree. This prevents the roster from creating a second person for a typo.
...
function likelyNameTypo(a, b) {
  const af = normalizeIdentity(a.firstName);
  const bf = normalizeIdentity(b.firstName);
  const al = normalizeIdentity(a.lastName);
  const bl = normalizeIdentity(b.lastName);
  if (!af || !bf || !al || !bl || al !== bl) return false;
  if (af === bf) return true;
  if (Math.max(af.length, bf.length) < 4 || Math.abs(af.length - bf.length) > 1) return false;
  if (editDistance(af, bf) > 1) return false;

  const at = normalizeIdentity(a.team);
  const bt = normalizeIdentity(b.team);
  const as = normalizeIdentity(a.school);
  const bs = normalizeIdentity(b.school);
  const ac = normalizeIdentity(a.city);
  const bc = normalizeIdentity(b.city);
  return (at && bt && at === bt) || (as && bs && as === bs) || (ac && bc && ac === bc);
}
...
function buildCombinedDataset(importedRaces) {
  ...
  races.forEach((race) => race.results.forEach((r) => {
    const originalId = r.athleteId;
    let id = canonicalIds.get(originalId) || originalId;
    let existing = registry.get(id);

    if (!existing) {
      // Also look for a near-identical name already registered. This catches
      // small source typos without merging unrelated athletes.
      for (const [candidateId, candidate] of registry.entries()) {
        if (likelyNameTypo(r, candidate)) {
          id = candidateId;
          canonicalIds.set(originalId, id);
          existing = candidate;
          break;
        }
      }
    ...
    if (!existing) {
      registry.set(id, {
        ...
        gender: r.gender || "", field: r.field || "", sourceAthleteId: r.sourceAthleteId || "",
        _nameCounts: { [normalizeIdentity(`${r.firstName || ""} ${r.lastName || ""}`)]: { count: 1, firstName: r.firstName || "", lastName: r.lastName || "" } },
      });
    } else {
      const name = normalizeIdentity(`${r.firstName || ""} ${r.lastName || ""}`);
      const counts = { ...(existing._nameCounts || {}) };
      const previous = counts[name] || { count: 0, firstName: r.firstName || "", lastName: r.lastName || "" };
      counts[name] = { count: previous.count + 1, firstName: previous.firstName || r.firstName || "", lastName: previous.lastName || r.lastName || "" };
      const best = Object.values(counts).sort((a, b) => b.count - a.count)[0] || counts[name];
      registry.set(id, {
        ...existing,
        firstName: best.firstName || existing.firstName,
        lastName: best.lastName || existing.lastName,
  ...
  const athletes = [...registry.values()].map(({ _nameCounts, ...athlete }) => athlete);
