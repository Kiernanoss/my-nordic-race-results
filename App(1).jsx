...
function athleteIdentityKey(a) {
  // Use the normalized full name as the stable identity. Some Endurance
  // result rows are incomplete (for example, the same athlete may appear
  // once with a team and once without one). Including school/team/city in
  // the identity would incorrectly create multiple athletes for one person.
  const name = normalizeIdentity(`${a.firstName || ""} ${a.lastName || ""}`);
  return `person:${name}`;
}

function resultCompletenessScore(r) {
  // When the same athlete appears more than once in one race, prefer the
  // richest row. This prevents an incomplete duplicate from replacing the
  // real result/placing.
  return (r.team ? 8 : 0) +
    (r.school ? 4 : 0) +
    (r.class || r.field ? 2 : 0) +
    (r.bib ? 1 : 0) +
    (Number.isFinite(r.timeSec) ? 1 : 0) +
    (r.place && !Number.isNaN(parseInt(r.place, 10)) ? 1 : 0);
}
...
function parseScraperExport(text) {
  ...
  matches.forEach((m) => {
    ...
    rows.forEach((r) => {
      ...
      const nameKey = normalizeIdentity(`${norm.firstName || ""} ${norm.lastName || ""}`);
      // Keep separate fields/distances, but collapse duplicate copies of the
      // same athlete inside a field. Prefer the most complete source row.
      const dedupeKey = `${nameKey}|${normalizeIdentity(norm.field || norm.class || "")}|${norm.distanceKm ?? "unknown"}`;
      const existingIndex = race.rawRows.findIndex((x) => {
        const xName = normalizeIdentity(`${x.firstName || ""} ${x.lastName || ""}`);
        return `${xName}|${normalizeIdentity(x.field || x.class || "")}|${x.distanceKm ?? "unknown"}` === dedupeKey;
      });
      if (existingIndex < 0) {
      ...
      const newScore = resultCompletenessScore(norm);
      const oldScore = resultCompletenessScore(existing);
      const newPlace = parseInt(norm.place, 10);
      const oldPlace = parseInt(existing.place, 10);
      if (newScore > oldScore || (newScore === oldScore && Number.isFinite(newPlace) && (!Number.isFinite(oldPlace) || newPlace < oldPlace))) {
        race.rawRows[existingIndex] = norm;
      }
    });
  });
