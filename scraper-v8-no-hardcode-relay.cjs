...
async function parseResultResource(url, linkText, visited = new Set(), depth = 0) {
  ...
  try {
    ...
    if (isPdf) {
      ...
      const genderHint = inferGenderFromText(`${linkText} ${parsed.text}`);
      const fieldHint = inferFieldFromText(parsed.text) || clean(linkText);
      const rows = parseText(parsed.text, genderHint, fieldHint);
      return rows.length ? rows : parseRelayText(parsed.text, genderHint, fieldHint);
    }
