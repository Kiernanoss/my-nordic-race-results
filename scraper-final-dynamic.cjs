...
const DEFAULTS = {
  ...
  // Your first race is the starting boundary for discovery.
  // It can still be overridden with --start-id in GitHub Actions.
  startId: 1647,
...
function looksNordic(r) {
  ...
  // A page with Endurance Promotions' own field filter is a strong structural
  // signal that it is a result page worth processing, even if the site's
  // metadata labels the race type incorrectly. This is intentionally generic:
  // no RaceKey, event name, or field/category is hard-coded here.
  if (r.hasFieldFilter) {
    return true;
  }
...
function raceMeta(html, id) {
  ...
  // Some Endurance Promotions result pages are mislabeled with a non-Nordic
  // race type even though their result page exposes the site's field filter.
  // Keep this as page-structure detection rather than tying the scraper to a
  // particular RaceKey, event name, or category.
  const hasFieldFilter = /filter\s+by\s+field\s*:/i.test(text);
 ...
 * DISCOVER RACES
 *
 * Discovers result pages starting at the configured start-id.
 * No race-specific IDs are required by the scraper.
 */
...
async function processRace(race, a, stats, browser) {
   ...
   * GridView pager. The default grid can contain multiple result fields
   * across its pages. Earlier versions tried to discover the pager
  ...
  try {
    ...
    // The normal GridView can contain multiple result fields and paginates
    // the complete result set. Do not hard-code a particular field selection.
    ...
    console.log('  FIELD FILTER: scraping the complete result grid');
    await scrapeAllGridPages();

    // Some result pages expose their field selector through Telerik instead
    // of a normal HTML <select>. If the normal grid is empty, discover the
    // Telerik control and its actual items from the live page. Nothing about
    // a particular RaceKey, event, or field name is hard-coded here.
    //
    // Telerik documents findItemByText()/get_items()/get_selectedItem() and
    // item.select() for these controls; we use those APIs when available.
    //
    if (all.length === 0) {
      const discoverTelerikFieldControls = async () => {
        try {
          return await page.evaluate(() => {
            ...
            const inspect = (id) => {
              if (seen.has(id) || typeof window.$find !== 'function') return;
              seen.add(id);

              let control = null;
              try { control = window.$find(id); } catch {}
              if (!control || typeof control.get_items !== 'function') return;

              let collection;
              try { collection = control.get_items(); } catch { return; }
              if (!collection || typeof collection.get_count !== 'function' || typeof collection.getItem !== 'function') return;

              let count = 0;
              try { count = Number(collection.get_count()) || 0; } catch {}
              ...
              for (let i = 0; i < count; i++) {
                try {
                  ...
                  if (!item) continue;
                  const text = typeof item.get_text === 'function' ? String(item.get_text() || '').trim() : '';
                  const value = typeof item.get_value === 'function' ? String(item.get_value() || '').trim() : '';
                  if (text || value) items.push({ text, value, index: i });
                } catch {}
              }

              if (items.length < 2) return;

              // Ignore obvious numeric/page-size controls. Result-field
              // selectors are represented by textual items.
              const textual = items.filter(x => x.text && !/^[-+]?\\d+(?:\\.\\d+)?$/.test(x.text));
              if (textual.length < 2) return;

              out.push({ id, items });
            };

            for (const id of ids) inspect(id);

            // Some Telerik controls expose a client object without a useful
            // DOM id in the obvious element list. Inspect the common Telerik
            // client collections when they exist as an additional discovery
            // path, still without assuming a control id.
            try {
              const comboBoxes = window.Telerik?.Web?.UI?.RadComboBox?.ComboBoxes || [];
              for (const combo of comboBoxes) {
                if (!combo || typeof combo.get_id !== 'function') continue;
                inspect(combo.get_id());
              }
            } catch {}
          ...
          console.warn(`  FIELD FALLBACK: could not inspect Telerik controls: ${error.message}`);
          return [];
        }
      };

      const selectTelerikItem = async (controlId, itemIndex) => {
        try {
          return await page.evaluate(({ controlId, itemIndex }) => {
            if (typeof window.$find !== 'function') return { ok: false, reason: '$find unavailable' };
            const control = window.$find(controlId);
            if (!control || typeof control.get_items !== 'function') {
              return { ok: false, reason: 'Telerik control not found' };
            }

            const items = control.get_items();
            const item = items && typeof items.getItem === 'function' ? items.getItem(itemIndex) : null;
            if (!item || typeof item.select !== 'function') {
              return { ok: false, reason: 'Telerik item cannot be selected' };
            }

            const text = typeof item.get_text === 'function' ? String(item.get_text() || '') : '';
            const value = typeof item.get_value === 'function' ? String(item.get_value() || '') : '';
            item.select();

            // For controls that require the actual DOM interaction to raise
            // their configured postback/callback, click the selected item's
            // element as a second, browser-native path.
            try {
              const el = typeof item.get_element === 'function' ? item.get_element() : null;
      ...
      const controls = await discoverTelerikFieldControls();

      if (controls.length) {
        // Prefer the control with the fewest textual choices. That normally
        // identifies the race field selector while avoiding unrelated menus.
        controls.sort((a, b) => {
          const at = a.items.filter(x => x.text && !/^[-+]?\\d+(?:\\.\\d+)?$/.test(x.text)).length;
          const bt = b.items.filter(x => x.text && !/^[-+]?\\d+(?:\\.\\d+)?$/.test(x.text)).length;
          return at - bt;
        });

        const fieldControl = controls[0];
        const candidates = fieldControl.items.filter(x => x.text || x.value);

        console.log(`  FIELD FALLBACK: discovered Telerik field control ${fieldControl.id} with ${candidates.length} options.`);

        // Re-open the race from scratch for each discovered option. This is
        // important because ASP.NET/Telerik postbacks rebuild the control and
        // can invalidate the old client object after one selection.
        for (const option of candidates) {
          await page.goto(race.url, { waitUntil: 'domcontentloaded', timeout: a.timeout });
          await page.waitForTimeout(500);

          const freshControls = await discoverTelerikFieldControls();
          const fresh = freshControls.find(c => c.id === fieldControl.id) || freshControls[0];
          if (!fresh) {
            console.warn(`  FIELD FALLBACK: Telerik field control disappeared before option ${option.text || option.value}.`);
            continue;
          }

          const freshIndex = fresh.items.findIndex(x =>
            (option.value && x.value === option.value) ||
            (!option.value && x.text === option.text)
          );
          if (freshIndex < 0) {
            console.warn(`  FIELD FALLBACK: option ${option.text || option.value} was not found after reload.`);
            continue;
          }

          const selected = await selectTelerikItem(fresh.id, freshIndex);
          if (!selected.ok) {
            console.warn(`  FIELD FALLBACK: could not select ${option.text || option.value}: ${selected.reason}`);
            continue;
          }

          console.log(`  FIELD FALLBACK: selected ${selected.text || selected.value}`);

          // Allow full postbacks, Telerik AJAX callbacks, and delayed grid
          // rendering to finish before parsing the result table.
          await waitForPostback();
        ...
        console.log('  FIELD FALLBACK: no dynamic Telerik field selector was discovered.');
      }
    }
...
async function main() {
   ...
   * STEP 1
   *
   * Discover RaceKeys beginning at the configured start-id.
   */
  const races =
