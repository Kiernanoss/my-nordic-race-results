  ...
  const scrapeAllGridPages = async () => {
    ...
    const findPagerControl = async (wantedPage) => {
      return page.evaluate((wanted) => {
        ...
        // Endurance Promotions uses a Telerik RadGrid. Prefer the exact
        // page-number link inside THIS grid so we cannot accidentally click
        // another "2" elsewhere on the page.
        const exact = [...grid.querySelectorAll('.rgNumPart a')].find(el =>
          (el.textContent || '').trim() === targetText
        );
        if (exact) {
          return {
            mode: 'exact-grid-click',
            outerHTML: exact.outerHTML.slice(0, 1200)
          };
        }

        // Keep the older postback fallback for pages whose markup differs.
        const elements = [...grid.querySelectorAll('a, input, button, span')];
        const attrValue = (el, name) => el.getAttribute(name) || '';
        ...
        for (const el of elements) {
          ...
          for (const candidate of [
            ...
            if (post) return { mode: 'postback', target: post.target, argument: post.argument };
          }
          if (el.tagName === 'A' || el.tagName === 'INPUT' || el.tagName === 'BUTTON') {
            return { mode: 'click', outerHTML: el.outerHTML.slice(0, 1200) };
          }
        }
        return null;
      }, wantedPage);
    ...
    const waitForGridPage = async (wantedPage) => {
      try {
        await page.waitForFunction((wanted) => {
          const grid = document.querySelector('#ctl00_cphMain_grdIndividualResults');
          if (!grid) return false;
          const current = grid.querySelector('.rgNumPart a.rgCurrentPage, .rgNumPart a[class*="rgCurrentPage"]');
          const currentText = current?.textContent?.trim() || '';
          let asyncBusy = false;
          try {
            const prm = window.Sys?.WebForms?.PageRequestManager?.getInstance?.();
            asyncBusy = !!prm?.get_isInAsyncPostBack?.();
          } catch {}
          return currentText === String(wanted) && !asyncBusy;
        }, wantedPage, { timeout: Math.min(a.timeout, 15000), polling: 100 });
        return true;
      } catch {
        return false;
      }
    ...
    const triggerNextPage = async (nextPage) => {
      ...
      if (control.mode === 'exact-grid-click') {
        // Use DOM click (not Playwright locator.click) so a footer that is
        // below the current viewport cannot trigger an "element not visible"
        // timeout. The click still executes the site's own Telerik/ASP.NET
        // JavaScript exactly as authored by Endurance Promotions.
        const clicked = await page.evaluate((wanted) => {
          const grid = document.querySelector('#ctl00_cphMain_grdIndividualResults');
          const el = [...(grid?.querySelectorAll('.rgNumPart a') || [])].find(a =>
            (a.textContent || '').trim() === String(wanted)
          );
          if (!el) return false;
          el.click();
          return true;
      ...
      // For Telerik's AJAX RadGrid, the strongest signal that the postback
      // actually completed is that the grid's current-page anchor changed.
      // Do this before parsing the grid so we never mistake the old page for
      // a new page just because networkidle fired early.
      await waitForGridPage(nextPage);
      return true;
    };
