...
function looksNordic(r) {
  ...
  const text = norm(`${r.event} ${r.location} ${r.description}`);

  if (raceType === 'skiing') return true;

  if (NORDIC_KEYWORDS.some(k => text.includes(norm(k)))) return true;

  // Some Nordic relay pages are mislabeled as Cycling. Use the result
  // documents exposed by the page rather than a hard-coded RaceKey.
  const resultText = norm((r.resultLinks || []).map(x => x.text).join(' '));
  if (/\b(?:boys?|girls?)\b/.test(resultText) &&
      /\bclassic\b/.test(resultText) &&
      /\brelay\b/.test(resultText)) return true;

  if (text.includes('classic') && (text.includes('ski') || text.includes('cross country') || text.includes('nordic'))) return true;
  if (text.includes('pursuit') && (text.includes('ski') || text.includes('cross country') || text.includes('nordic'))) return true;
...
function parseRelayText(text, genderHint = null, fieldHint = null) {
  ...
  for (const raw of String(text || '').split(/\r?\n/)) {
    const s = clean(raw);
    if (!s || /^place\s+/i.test(s) || /^=+/.test(s)) continue;

    const tokens = s.split(/\s+/);
    if (tokens.length < 8 || !/^\d+$/.test(tokens[0]) || !/^\d+$/.test(tokens[1])) continue;

    // Endurance relay PDFs use Place/Pts/(Bib)/team name/field/laps/Time.
    // Find the trailing run of lap/time columns instead of assuming a fixed
    // number of laps.
    let i = tokens.length - 1;
    if (!/^\d{1,3}:\d{2}(?:\.\d+)?$/.test(tokens[i])) continue;
    let timeCount = 0;
    while (i >= 2 && /^\d{1,3}:\d{2}(?:\.\d+)?$/.test(tokens[i])) {
      timeCount++;
      i--;
    }
    if (timeCount < 2) continue;

    const totalTime = tokens[tokens.length - 1];
    const middle = tokens.slice(2, i + 1);
    let start = 0;
    let bib = '';
    if (middle[0] && /^\d+$/.test(middle[0])) {
      bib = middle[0];
      start = 1;
    }

    let preTeam = middle.slice(start);
    if (preTeam.length && /^(?:x|f)$/i.test(preTeam[preTeam.length - 1])) preTeam.pop();
    if (preTeam.length < 2) continue;

    const school = preTeam[preTeam.length - 1];
    const relayName = preTeam.slice(0, -1).join(' ');
    if (!relayName || !/\s&\s/i.test(relayName)) continue;

    const n = splitName(relayName);
    out.push({
      firstName: n.firstName,
      lastName: n.lastName,
      name: relayName,
      ...
      age: null,
      bib: bib || null,
      place: Number(tokens[0]),
      time: totalTime,
      timeSeconds: timeSec(totalTime),
    ...
    const results =
      await mapLimit(
        ...
        async id => {
          ...
          const resultLinks = parseLinks(response.body)
            .filter(link => /(?:result|relay|pursuit|skate|classic)/i.test(link.text));
...
async function processRace(race, a, stats, browser) {
  ...
  try {
    ...
    // Some Endurance pages have a live field selector; others publish their
    // actual results as downloadable TXT/PDF documents. Handle both generically
    // rather than special-casing a particular race or field names.
    ...
    for (const link of links) {
      if (/\.txt(?:\?|$)/i.test(link.href)) {
        ...
        const genderHint = inferGenderFromText(`${link.text} ${response.body}`);
        const fieldHint = inferFieldFromText(response.body) || clean(link.text);
        const rows = parseText(response.body, genderHint, fieldHint);
        const relayRows = rows.length ? [] : parseRelayText(response.body, genderHint, fieldHint);
        const parsedRows = rows.length ? rows : relayRows;
        if (parsedRows.length) { addRows(parsedRows, link.href); stats.txt++; }
        ...
        try {
          ...
          const genderHint = inferGenderFromText(`${link.text} ${parsed.text}`);
          const fieldHint = inferFieldFromText(parsed.text) || clean(link.text);
          const rows = parseText(parsed.text, genderHint, fieldHint);
          const relayRows = rows.length ? [] : parseRelayText(parsed.text, genderHint, fieldHint);
          const parsedRows = rows.length ? rows : relayRows;
          if (parsedRows.length) { addRows(parsedRows, link.href); stats.pdf++; }
        } catch {}
      }
    }
