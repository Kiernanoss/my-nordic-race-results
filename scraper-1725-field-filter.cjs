...
function looksNordic(r) {
  ...
  // RaceKey 1725 is the 1/13/2026 MEC Classic Varsity Relays.
  // Endurance Promotions labels it Cycling even though it belongs to the
  // MEC Nordic season. Keep the race page in the dataset for now even when
  // its result grid is empty; we will fix the result extraction separately.
  if (String(r.id) === '1725') {
    return true;
  }
...
async function processRace(race, a, stats, browser) {
  ...
  const scrapeAllGridPages = async () => {
    ...
    const findPagerControl = async (wantedPage) => {
      return page.evaluate(({ wanted, special1725 }) => {
        const targetText = String(wanted);

        // RaceKey 1725 uses Telerik RadGrid markup whose page links live in
        // .rgNumPart and use the site's own javascript: doPostBack(...) hrefs.
        // Keep this special handling isolated to 1725 so the proven baseline
        // pagination remains unchanged for every other race.
        if (special1725) {
          const grid = document.querySelector('#ctl00_cphMain_grdIndividualResults');
          const pager = grid?.querySelector('.rgNumPart');
          const anchor = pager ? [...pager.querySelectorAll('a')].find(el => {
            const text = (el.textContent || '').trim();
            return text === targetText && !el.classList.contains('rgCurrentPage');
          }) : null;
          if (anchor) {
            return {
              mode: 'radgrid-javascript',
        ...
        return null;
      }, { wanted: wantedPage, special1725: String(race.id) === '1725' });
    };
    ...
    const triggerNextPage = async (nextPage) => {
      ...
      if (control.mode === 'radgrid-javascript') {
        const href = String(control.href || '').trim();
        if (!href.toLowerCase().startsWith('javascript:')) {
          console.warn(`  GRID: RaceKey 1725 page ${nextPage} is missing its RadGrid javascript postback; stopping.`);
          return false;
        }
        ...
        try {
          await page.waitForFunction((wanted) => {
            const grid = document.querySelector('#ctl00_cphMain_grdIndividualResults');
            const current = grid?.querySelector('.rgNumPart a.rgCurrentPage');
            return !!current && (current.textContent || '').trim() === String(wanted);
          }, nextPage, { timeout: Math.min(a.timeout, 10000) });
        } catch {}
  ...
  try {
    ...
    // RaceKey 1725 is different from the normal MEC/IMAC pages: the initial
    // combined grid can be empty even though the Telerik RadGrid contains
    // results behind its field dropdown.  Handle ONLY 1725 here so the known
    // good 2025-26 boys/girls scraping remains untouched everywhere else.
    if (String(race.id) === '1725' && all.length === 0) {
      const select1725Field = async (fieldText) => {
        const selectors = [
          '#ctl00_cphMain_ddlIndividualResultsField',
          '[id$="_ddlIndividualResultsField"]',
          '[id*="IndividualResultsField"]'
        ];
        ...
        try {
          ...
          try {
            await control.locator('span, input, div').first().click();
          } catch {
            return false;
          }
        }
        ...
        // Last resort: exact text click. At this point the dropdown is open
        // and the result grid is empty, so the exact field label is unambiguous.
        try {
          const exact = page.getByText(fieldText, { exact: true }).last();
      ...
      for (const fieldName of fieldNames) {
        ...
        const sizeAfterField = await findPageSizeSelect();
        if (sizeAfterField) {
          try {
            await page.locator('select').nth(sizeAfterField.index).selectOption({ label: '50' });
            await waitForPostback();
          } catch {}
        }
