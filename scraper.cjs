...
async function discoverRacePages(page, timeout) {
  ...
  if (discovered.size === 0) {
    ...
    for (let safety = 0; safety < totalPages + 2; safety += 1) {
      const data = await extractRaceIndexPage(page, page.url());
      addRows(data);
      const next = await page.locator('a,button,input').evaluateAll(els => els.map((el, i) => ({
        i, text: clean(el.textContent || el.value || ''), href: el.getAttribute('href') || '',
        onclick: el.getAttribute('onclick') || '', disabled: !!el.disabled
      })).find(x => !x.disabled && /next|forward|right/i.test(`${x.text} ${x.href} ${x.onclick}`)));
      if (!next) break;
      try { await page.locator('a,button,input').nth(next.i).click({ timeout: 10000 }); await page.waitForTimeout(500); }
      catch { break; }
    }
  }
