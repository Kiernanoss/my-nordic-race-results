  ...
  const scrapeAllGridPages = async () => {
    ...
    const findPagerControl = async (wantedPage) => {
      return page.evaluate((wanted) => {
        const root = document.querySelector('#ctl00_cphMain_grdIndividualResults');
        if (!root) return null;

        const links = [...root.querySelectorAll('tfoot .rgPager .rgNumPart a, .rgPager .rgNumPart a')];
        const candidates = links.filter(a => (a.textContent || '').trim() === String(wanted));
        if (!candidates.length) return null;

        // Prefer a truly visible link. Some Telerik layouts leave duplicate
        // pager anchors in the DOM that are hidden; Playwright click() would
        // wait forever trying to scroll those into view. DOM click() bypasses
        // that problem while still executing the page's real href/javascript.
    ...
    const getCurrentRadPage = async () => {
      return page.evaluate(() => {
        const root = document.querySelector('#ctl00_cphMain_grdIndividualResults');
        if (!root) return null;
        const current = root.querySelector('.rgPager .rgCurrentPage');
        return current ? (current.textContent || '').trim() : null;
      });
    };

    const triggerNextPage = async (nextPage, previousSignature) => {
      const control = await findPagerControl(nextPage);

      if (!control) {
        console.warn(`  GRID: could not locate RadGrid pager link for page ${nextPage}; stopping.`);
        return false;
      }

      console.log(`  GRID: exact RadGrid page ${nextPage} link: ${control.outerHTML}`);

      const previousRadPage = await getCurrentRadPage();

      // IMPORTANT: use a real DOM click on the exact Telerik anchor.
      // Do NOT call window.__doPostBack() ourselves and do NOT use eval().
      // The anchor's javascript href is the site's own pagination mechanism.
      const clicked = await page.evaluate((wanted) => {
        const root = document.querySelector('#ctl00_cphMain_grdIndividualResults');
        if (!root) return false;
        const links = [...root.querySelectorAll('tfoot .rgPager .rgNumPart a, .rgPager .rgNumPart a')]
          .filter(a => (a.textContent || '').trim() === String(wanted));
      ...
      // Telerik uses ASP.NET AJAX postbacks, so networkidle is not a reliable
      // signal. Wait for either the current-page marker to change or the main
      // result rows to change. Retry the observation for several seconds.
      const deadline = Date.now() + Math.max(5000, Math.min(12000, a.timeout));
      while (Date.now() < deadline) {
        await page.waitForTimeout(250);

        const currentPage = await getCurrentRadPage();
        if (currentPage === String(nextPage)) return true;

        const html = await page.content();
        const tables = parseTables(html)
          .filter((table) => findResultHeader(table) >= 0)
          .sort((x, y) => y.length - x.length);
        const rows = rowsFromTable(tables[0] || []);
        const signature = rows.slice(0, 5).map(r =>
          `${r.firstName}|${r.lastName}|${r.field}|${r.place}|${r.time}`
        ).join('||');
    ...
    while (pageNo <= totalPages && pageNo <= 200) {
      ...
      const moved = await triggerNextPage(nextPage, current.signature);
      if (!moved) {
        console.warn(`  GRID: RadGrid did not move to page ${nextPage}; stopping this race.`);
        break;
      }
