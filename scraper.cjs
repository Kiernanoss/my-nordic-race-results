...
async function processRace(race, a, stats, browser) {
  ...
  const scrapeAllPagesForCurrentField = async (fieldGender) => {
    ...
    while (pageNo <= totalPages && pageNo <= 200) {
      ...
      const postback = await page.evaluate((wanted) => {
        const elements = [...document.querySelectorAll('a, input, button')];
        for (const el of elements) {
          const s = `${el.getAttribute('href') || ''} ${el.getAttribute('onclick') || ''} ${el.getAttribute('value') || ''}`;
          const re = new RegExp(`__doPostBack\\(\\s*['\\\"]([^'\\\"]+)['\\\"]\\s*,\\s*['\\\"]Page\\$${wanted}['\\\"]\\s*\\)`);
          const match = s.match(re);
          if (match) return { target: match[1], argument: `Page$${wanted}` };
        }
        return null;
      }, nextPage);
      ...
      if (postback) {
        ...
        const link = page.getByRole('link', { name: String(nextPage), exact: true }).last();
        if (await link.count()) {
          await link.click();
          await waitForPostback();
        } else {
          console.warn(`  GRID: could not locate pager for page ${nextPage}; stopping this field.`);
          break;
        }
      }
      pageNo = nextPage;
...
async function main() {
  ...
  console.log(
    'Endurance Promotions Nordic scraper v7.2'
  );
