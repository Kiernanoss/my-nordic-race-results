...
// Endurance Promotions PDFs sometimes contain unsupported TrueType hints.
// pdf-parse/pdf.js reports these as harmless 'TT: undefined function: N'
// warnings even when text extraction succeeds. Keep other warnings visible.
const _originalConsoleWarn = console.warn.bind(console);
console.warn = (...args) => {
  const msg = args.map(v => String(v)).join(' ');
  if (/TT: undefined function:\s*\d+/i.test(msg)) return;
...
async function processRace(race, a, stats, browser) {
  ...
  const nordicFieldInfo = async () => {
    const selects = await selectInfo();
    const known = new Set(['BJV','BVAR','GJV','GVAR','B6TH','G6TH','BMS','GMS','MS6','MIDDLE SCHOOL_6TH']);
    return selects.find(s => {
      const haystack = s.options.flatMap(o => [o.text, o.value])
        .map(v => clean(v).toUpperCase());
      const hits = haystack.filter(v => known.has(v) || /^(B|G)(JV|VAR|6TH|MS)$/.test(v) || /^MIDDLE SCHOOL/.test(v));
      return hits.some(v => /^B(JV|VAR|6TH|MS)$/.test(v)) && hits.some(v => /^G(JV|VAR|6TH|MS)$/.test(v));
    }) || null;
  };

  const fieldOptions = async () => {
    const info = await nordicFieldInfo();
    if (!info) return [];
    ...
    for (const option of info.options) {
      ...
      const isField = /^(B|G)(JV|VAR|6TH|MS)$/.test(upper) ||
        /^(B|G)(JV|VAR|6TH|MS)$/.test(valueUpper) ||
        /^MIDDLE SCHOOL/.test(upper) || /^MIDDLE SCHOOL/.test(valueUpper);
      if (!isField || !value || seenValues.has(value)) continue;
  ...
  try {
    ...
    if (fields.length) {
      console.log(`  FIELD FILTER: detected ${fields.length} result fields: ${fields.map(f => f.text).join(', ')}`);
      // IMAC/MEC relay and mixed-class pages can paginate unreliably as one
      // combined grid. Scrape each actual Field option from a fresh page so
      // every class is retrieved independently, then deduplicate.
      for (const field of fields) {
        try {
          await page.goto(race.url, { waitUntil: 'domcontentloaded', timeout: a.timeout });
          await page.waitForTimeout(350);
          const freshFields = await fieldOptions();
          if (!freshFields.length) {
            console.warn(`  FIELD ${field.text}: selector disappeared after reload; skipping.`);
            continue;
          }
          const fresh = freshFields.find(f => f.value === field.value || f.text === field.text);
          if (!fresh) {
            console.warn(`  FIELD ${field.text}: option not found after reload; skipping.`);
            continue;
          }
          const info = await nordicFieldInfo();
          const selectLocator = page.locator('select').nth(info.index);
          await selectLocator.selectOption(fresh.value);
          await waitForPostback();
          console.log(`  FIELD ${field.text}: scraping selected result grid`);
          await scrapeAllGridPages();
        } catch (e) {
          console.warn(`  FIELD ${field.text}: ${e.message}`);
        }
      }
    } else {
