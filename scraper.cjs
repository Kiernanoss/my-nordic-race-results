...
async function processRace(race, a, stats, browser) {
   ...
   */
  const scrapeAllGridPagesForCurrentField = async (fieldLabel) => {
    let previousSignature = '';
    let pageNo = 1;
    let totalPages = 1;

    const parseMainGridForField = async () => {
      const html = await page.content();
      const tables = parseTables(html)
    ...
    const triggerNextPage = async (nextPage) => {
      ...
      if (!control) {
        console.warn(`  FIELD ${fieldLabel}: could not locate pager control for page ${nextPage}; stopping this field.`);
        return false;
      }
    ...
    while (pageNo <= totalPages && pageNo <= 200) {
      const current = await parseMainGridForField();
      ...
      console.log(`  FIELD ${fieldLabel}: page ${pageNo}/${totalPages} -> ${current.count} new rows`);

      if (current.signature && current.signature === previousSignature) {
        console.warn(`  FIELD ${fieldLabel}: page ${pageNo} repeated the previous page; stopping this field.`);
        break;
      }
      ...
      if (!(await triggerNextPage(nextPage))) break;
      pageNo = nextPage;
    }
  };

  const scrapeByField = async () => {
    const initialFieldSelect = await findFieldSelect();
    if (!initialFieldSelect || !initialFieldSelect.options.length) {
      console.log('  FIELD FILTER: no BJV/BVAR/GJV/GVAR selector found; using complete grid pagination.');
      // Preserve the current working behavior as a fallback.
      await scrapeAllGridPagesForCurrentField('ALL');
      return;
    }

    const options = initialFieldSelect.options.filter(o => {
      const t = String(o.text || '').trim().toUpperCase();
      const v = String(o.value || '').trim().toUpperCase();
      return /^(BJV|BVAR|GJV|GVAR)$/.test(t) || /^(BJV|BVAR|GJV|GVAR)$/.test(v);
    });

    if (!options.length) {
      console.log('  FIELD FILTER: selector found but no BJV/BVAR/GJV/GVAR options; using complete grid pagination.');
      await scrapeAllGridPagesForCurrentField('ALL');
      return;
    }

    console.log(`  FIELD FILTER: scraping ${options.map(o => o.text).join(', ')}`);
    ...
    for (const option of options) {
      ...
      if (!fieldSelect) {
        console.warn(`  FIELD ${option.text}: selector disappeared after reload; skipping field.`);
        continue;
      }

      try {
        const select = page.locator('select').nth(fieldSelect.index);
        await select.selectOption({ value: option.value });
        await waitForPostback();
      } catch (e) {
        console.warn(`  FIELD ${option.text}: could not select field: ${e.message}`);
        continue;
      }

      // Set page size to 50 after selecting the field. If this postback resets
      // the field, reload and re-select the field before scraping.
      ...
      if (sizeSelect) {
        try {
          ...
          const verify = await findFieldSelect();
          if (verify) {
            const selectedText = await page.locator('select').nth(verify.index).locator('option:checked').textContent().catch(() => '');
            if (String(selectedText || '').trim().toUpperCase() !== String(option.text).trim().toUpperCase()) {
              await page.goto(race.url, { waitUntil: 'domcontentloaded', timeout: a.timeout });
              await page.waitForTimeout(350);
              const verifyAfterReload = await findFieldSelect();
              if (verifyAfterReload) {
                await page.locator('select').nth(verifyAfterReload.index).selectOption({ value: option.value });
                await waitForPostback();
              }
            }
          }
        } catch (e) {
          console.warn(`  FIELD ${option.text}: could not set page size to 50: ${e.message}`);
        }
      }

      await scrapeAllGridPagesForCurrentField(option.text);
    }
  };
