...
function looksNordic(r) {
  ...
  // RaceKey 1725 is labeled Cycling on Endurance Promotions, but it is the
  // MEC Classic Varsity Relays that belongs with the Nordic season. Keep the
  // race page in discovery so we can scrape its field-specific results.
  if (String(r.id) === '1725') {
    return true;
  }
 ...
 * DISCOVER RACES
 *
 * Starts at RaceKey 1666 by default so the 2024-2025 Nordic season is included alongside later seasons.
 *
 * This includes the 2024-2025 season while still retaining the current seasons.
 */
...
async function processRace(race, a, stats, browser) {
  ...
  const addRows = (rows, sourceUrl) => {
    for (const row of rows) {
      ...
      const enriched = {
        ...
        season: seasonForDate(race.date),
      };
      const key = [
  ...
  try {
    ...
    console.log('  FIELD FILTER: scraping the complete result grid (BJV/BVAR/GJV/GVAR)');
    await scrapeAllGridPages();

    // RaceKey 1725 has a Telerik RadDropDownList field selector. Unlike the
    // normal Nordic pages, its initial grid is empty until a field is selected.
    // Handle ONLY this race here. The normal 2025-26 grid/PDF/TXT logic above
    // remains unchanged for every other race.
    if (String(race.id) === '1725' && all.length === 0) {
      const controlId = 'ctl00_cphMain_ddlIndividualResultsField';
      const fieldNames = ['BJV', 'BVAR', 'GJV', 'GVAR'];

      const select1725Field = async (fieldText) => {
        try {
          const result = await page.evaluate(({ controlId, fieldText }) => {
            if (typeof window.$find !== 'function') {
              return { ok: false, reason: 'Telerik $find is not available.' };
            }

            const ddl = window.$find(controlId);
            if (!ddl) {
              return { ok: false, reason: `RadDropDownList ${controlId} was not found.` };
            }

            const item = typeof ddl.findItemByText === 'function'
              ? ddl.findItemByText(fieldText)
              : null;

            if (!item) {
              let available = [];
              try {
                const items = ddl.get_items();
                const count = items && typeof items.get_count === 'function'
                  ? items.get_count()
                  : 0;
                for (let i = 0; i < count; i++) {
                  const it = items.getItem(i);
                  if (it && typeof it.get_text === 'function') {
                    available.push(it.get_text());
                  }
                }
              } catch {}
              return {
                ok: false,
                reason: `Telerik item ${fieldText} was not found.`,
                available
              };
            }

            if (typeof item.select !== 'function') {
              return { ok: false, reason: `Telerik item ${fieldText} has no select() method.` };
            }

            // Telerik RadDropDownList exposes item.select(), which performs the
            // same client-side selection as choosing the item in the UI.
            item.select();

            return {
              ok: true,
              text: typeof item.get_text === 'function' ? item.get_text() : fieldText,
              value: typeof item.get_value === 'function' ? item.get_value() : ''
            };
          }, { controlId, fieldText });

          if (!result?.ok) {
            console.warn(`  FIELD FALLBACK: Telerik selection for ${fieldText} did not run: ${result?.reason || 'unknown reason'}`);
            if (result?.available?.length) {
              console.warn(`  FIELD FALLBACK: available Telerik fields: ${result.available.join(', ')}`);
            }
            return false;
          }

          // Selection can trigger an ASP.NET/Telerik async postback. Wait for
          // the selected value to be reflected in the rebuilt page before
          // parsing the result grid.
          try {
            await page.waitForFunction(
              ({ controlId, fieldText }) => {
                try {
                  const ddl = window.$find && window.$find(controlId);
                  const selected = ddl && ddl.get_selectedItem
                    ? ddl.get_selectedItem()
                    : null;
                  return !!selected &&
                    typeof selected.get_text === 'function' &&
                    selected.get_text() === fieldText;
                } catch {
                  return false;
                }
              },
              { controlId, fieldText },
              { timeout: Math.min(a.timeout, 8000) }
            );
          } catch {}
          ...
          console.warn(`  FIELD FALLBACK: Telerik API selection failed for ${fieldText}: ${error.message}`);
          return false;
        }
      };

      console.log('  FIELD FALLBACK: RaceKey 1725 uses the Telerik field dropdown; scraping BJV/BVAR/GJV/GVAR individually.');

      for (const fieldName of fieldNames) {
        // Start from a clean 1725 page for each field so one Telerik postback
        // cannot leave the next field in a stale client state.
        ...
        const selected = await select1725Field(fieldName);
        if (!selected) {
          console.warn(`  FIELD FALLBACK: could not select ${fieldName} on RaceKey 1725.`);
          continue;
        }

        console.log(`  FIELD FALLBACK: ${fieldName}`);
        await scrapeAllGridPages();
      }
    }
    ...
    stats.perRace.push({ raceKey: race.id, event: race.event, date: race.date, season: seasonForDate(race.date), rows: all.length, male, female, unknown });
...
async function main() {
  ...
  const metadata = {
    ...
    diagnostics: {
      ...
      seasons:
        [...new Set(stats.perRace.map(r => r.season).filter(Boolean))].sort(),
