  ...
  const scrapeAllGridPages = async () => {
    ...
    const findPagerControl = async (wantedPage) => {
      return page.evaluate((wanted) => {
        ...
        // IMPORTANT: only accept a control whose actual postback argument
        // is Page$N. Never click an arbitrary element whose visible text is
        // "2" because result rows, filters, and other controls can also
        // contain the same number. That was the reason several races were
        // silently repeating page 1.
        const candidates = [
          ...document.querySelectorAll('a, input, button, span')
        ];
        ...
        for (const el of candidates) {
          const attrs = [
            ...
            el.getAttribute('oncommand'),
            el.getAttribute('data-href')
          ];

          for (const raw of attrs) {
            const post = parsePostback(raw);
            if (post && post.argument.endsWith(target)) {
              matches.push({
                mode: 'postback',
                target: post.target,
                argument: post.argument,
                outerHTML: el.outerHTML.slice(0, 1200),
                priority: /grdIndividualResults/i.test(post.target) ? 0 : 1
              });
            }
          }
        }
        ...
        // Some ASP.NET GridViews render the pager link through a javascript:
        // URL that contains the postback after HTML decoding. Search the
        // entire live page source as a final, exact Page$N fallback.
        const html = document.documentElement.outerHTML || '';
        const escapedTarget = target.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const re = new RegExp(`__doPostBack\\s*\\(\\s*['"]([^'"]+)['"]\\s*,\\s*['"]([^'"]*${escapedTarget})['"]\\s*\\)`, 'i');
    ...
    const triggerNextPage = async (nextPage) => {
      ...
      if (control.mode !== 'postback') return false;
    ...
    while (pageNo <= totalPages && pageNo <= 200) {
      ...
      // Verify that the actual result grid changed. A successful postback
      // that leaves the same first rows is not a successful page advance.
      const beforeGridSignature = current.gridText;
      let changed = false;
      for (let attempt = 0; attempt < 10; attempt++) {
        await page.waitForTimeout(300);
        const checkSignature = await page.evaluate(() => {
          const tables = [...document.querySelectorAll('table')];
          const candidates = tables
            .map(t => {
              const trs = [...t.querySelectorAll('tr')];
              const cells = trs.map(tr => [...tr.querySelectorAll('td,th')].map(td => (td.textContent || '').trim()));
              const headerIndex = cells.findIndex(row => row.some(v => /First Name/i.test(v)) && row.some(v => /Last Name/i.test(v)) && row.some(v => /Total Time|Time/i.test(v)));
              return { rowCount: trs.length, cells, headerIndex };
            })
            .filter(x => x.headerIndex >= 0)
            .sort((a, b) => b.rowCount - a.rowCount);
          const main = candidates[0];
          if (!main) return '';
          const header = main.cells[main.headerIndex];
          const idx = name => header.findIndex(v => String(v).replace(/\s+/g, '').toLowerCase() === name);
          const first = idx('firstname');
          const last = idx('lastname');
          const field = idx('field');
          const place = Math.max(idx('pos'), idx('place'));
          const time = Math.max(idx('totaltime'), idx('time'));
          return main.cells.slice(main.headerIndex + 1)
            .filter(r => r.length && (r[first] || r[last]) && r[time])
            .slice(0, 5)
            .map(r => `${r[first] || ''}|${r[last] || ''}|${r[field] || ''}|${r[place] || ''}|${r[time] || ''}`)
            .join('||');
        });
        if (checkSignature && checkSignature !== beforeGridSignature) {
          changed = true;
          break;
        }
      }
      ...
      if (!changed) {
        console.warn(`  GRID: page ${nextPage} postback did not change the result grid; stopping.`);
        break;
      }
...
async function main() {
  ...
  console.log(
    'Endurance Promotions Nordic scraper v7.1'
  );
