...
const DEFAULTS = {
  ...
  // IMPORTANT:
  // RaceKey 1668 is the first race of the 2024-2025 Nordic season.
  // Keep the existing discovery/parsing logic intact; this simply lets the
  // working scraper collect that season alongside the current seasons.
  startId: 1668,
 ...
 * DISCOVER RACES
 *
 * Starts at RaceKey 1668 by default.
 *
...
function seasonForDate(rawDate) {
  if (!rawDate) return "Unknown Season";
  ...
  if (m) {
    ...
    const iso = String(rawDate).match(/(\d{4})-(\d{1,2})-(\d{1,2})/);
    if (!iso) return "Unknown Season";
    year = Number(iso[1]);
    month = Number(iso[2]);
  }
  // Nordic season: fall/winter of one calendar year through spring of the
  // next. Summer dates are assigned to the season beginning that fall.
...
async function main() {
   ...
   * STEP 1
   *
   * Discover RaceKeys beginning at 1668.
   */
  const races =
