...
async function parseResultResource(url, linkText, visited = new Set(), depth = 0) {
  ...
  try {
    ...
    if (isPdf) {
      ...
      return coordinateRows.length ? coordinateRows : parseRelayPdfText(coordinateText, coordinateGender, coordinateField);
    }
...
async function processRace(race, a, stats, browser) {
  ...
  try {
    ...
    for (const link of resultLinks) {
      const rows = await parseResultResource(link.href, link.text, visitedResources);
      if (rows.length) {
        addRows(rows, link.href);
        stats.pdf++;
      } else if (/relay/i.test(link.text)) {
        console.log(`  RESULT LINK: ${link.text} -> 0 parsed rows`);
      }
    }
