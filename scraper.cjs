...
function looksNordic(r) {
  ...
  // Some Nordic relay/conference pages are mislabeled as Cycling on
  // Endurance Promotions (for example IMAC/MEC relay pages). If the page
  // itself contains the Nordic high-school field signature, keep it.
  if (
    r.hasNordicFieldSignature &&
    (
      text.includes('mec') ||
      text.includes('imac') ||
      text.includes('nordic') ||
  ...
  // MEC/IMAC event titles are strong enough to retain even when the site
  // labels the Race Type as Cycling.
  if (r.hasMecImacSignature) {
    return true;
  }
...
function raceMeta(html, id) {
  ...
  const fieldSignature = /\bBJV\b/i.test(sourceHtml) && /\bBVAR\b/i.test(sourceHtml) && /\bGJV\b/i.test(sourceHtml) && /\bGVAR\b/i.test(sourceHtml);
  const eventText = `${title} ${description}`;
  const mecImacSignature = /\b(?:MEC|IMAC)\b/i.test(eventText) && /\b(?:nordic|ski|skate|classic|pursuit|relay|relays)\b/i.test(eventText);
  ...
  return {
    ...
    location,
    description,
    hasNordicFieldSignature: fieldSignature,
    hasMecImacSignature: mecImacSignature,
  };
}
...
async function processRace(race, a, stats, browser) {
  ...
  const findFieldSelect = async () => {
    ...
    const domMatch = selects.find(s => {
      const texts = s.options.map(o => norm(o.text));
      const hasB = texts.some(t => /^(?:b(?:jv|var|6th|ms)|boys?\b)/i.test(t));
      const hasG = texts.some(t => /^(?:g(?:jv|var|6th|ms)|girls?\b)/i.test(t));
      const hasFieldPlaceholder = texts.some(t => t === 'select a field' || t === 'show all');
      return hasB && hasG && hasFieldPlaceholder;
    });
    if (domMatch) return domMatch;

    // Some pages render the field selector in ASP.NET/Telerik markup that is
    // easier to recognize in the raw HTML than in the live select list.
    const raw = findFieldFilter(await page.content());
    if (raw) {
      const rawIndex = selects.findIndex(s =>
        (raw.id && s.id === raw.id) ||
        (raw.name && s.name === raw.name)
      );
      if (rawIndex >= 0) {
        return { ...selects[rawIndex], options: raw.options };
      }
    }
  ...
  try {
    ...
    if (fieldSelect) {
      const fieldOptions = fieldSelect.options.filter(o => {
        const t = norm(o.text);
        return t && t !== 'select a field' && t !== 'show all';
      });

      console.log(`  FIELD FILTER: detected ${fieldOptions.length} field option(s): ${fieldOptions.map(o => o.text).join(', ')}`);

      for (const option of fieldOptions) {
        // Fresh page for every field so each ASP.NET/Telerik postback starts
        // with clean ViewState/EventValidation state.
        await page.goto(race.url, { waitUntil: 'domcontentloaded', timeout: a.timeout });
        await page.waitForTimeout(350);

        const live = await findFieldSelect();
        if (!live) {
          console.warn(`  FIELD FILTER: could not restore selector for ${option.text}; skipping this field.`);
          continue;
        }

        try {
          await page.locator('select').nth(live.index).selectOption({ value: option.value });
          await waitForPostback();
          console.log(`  FIELD FILTER: ${option.text} -> scraping`);
          await scrapeAllGridPages();
        } catch (e) {
          console.warn(`  FIELD FILTER: ${option.text} failed: ${e.message}`);
        }
      }
    } else {
      console.log('  FIELD FILTER: no field selector found; scraping complete result grid.');
      await scrapeAllGridPages();
    }
     ...
     * RELAY FALLBACK:
     *
     * RaceKey 1725 (2026 MEC Classic Varsity Relays) is discovered correctly,
     * but its default result grid is empty.  These Endurance Promotions relay
     * pages can render "Filter by Field" as a custom Telerik/ASP.NET control
     * instead of a native <select>, so the normal field-selector detector sees
     * nothing.  When a MEC/IMAC Nordic-style race has produced zero rows, try
     * the field choices through the visible DOM instead.
     *
     * This is intentionally a fallback only; normal MEC/IMAC scraping above is
     * unchanged.
     */
    if (all.length === 0 && /\b(?:MEC|IMAC)\b/i.test(`${race.event} ${race.description}`)) {
      const relayFields = ['BJV', 'BVAR', 'GJV', 'GVAR'];
      ...
      const openFieldMenu = async () => {
        return page.evaluate(() => {
          ...
          const labels = [...document.querySelectorAll('*')]
            .filter(el => visible(el) && normText(el.textContent) === 'filter by field:')
            .sort((a, b) => a.getBoundingClientRect().width - b.getBoundingClientRect().width);

          for (const label of labels) {
            let box = label;
            for (let depth = 0; depth < 4 && box; depth++, box = box.parentElement) {
              const candidates = [...box.querySelectorAll('input,button,a,[role="button"],[role="combobox"],.rcbInput,.rcbArrowCell,.RadComboBox,span,div')]
                .filter(el => visible(el))
                .filter(el => normText(el.textContent) !== 'filter by field:');

              const scored = candidates.map(el => {
                const txt = normText(el.textContent);
                const own = normText(`${el.id || ''} ${el.className || ''} ${el.getAttribute('role') || ''}`);
                let score = 0;
                if (/combobox|dropdown|field|filter|rcb/.test(own)) score += 20;
                if (/select a field|show all|bjv|bvar|gjv|gvar/.test(txt)) score += 15;
                const r = el.getBoundingClientRect();
                const lr = label.getBoundingClientRect();
                if (Math.abs(r.top - lr.top) < 60) score += 5;
                if (r.width < 500) score += 2;
                return { el, score };
              }).sort((a,b) => b.score - a.score);

              if (scored.length && scored[0].score >= 15) {
                try {
                  scored[0].el.click();
                  return true;
                } catch {}
      ...
      const clickVisibleFieldOption = async (wanted) => {
        return page.evaluate((wantedText) => {
          ...
          const exact = [...document.querySelectorAll('*')]
            .filter(el => visible(el) && String(el.textContent || '').replace(/\s+/g, ' ').trim().toLowerCase() === wanted)
            .sort((a,b) => a.getBoundingClientRect().width - b.getBoundingClientRect().width);
      ...
      console.log('  RELAY FALLBACK: zero rows; attempting custom Filter by Field control.');

      for (const relayField of relayFields) {
        await page.goto(race.url, { waitUntil: 'domcontentloaded', timeout: a.timeout });
        await page.waitForTimeout(500);

        const opened = await openFieldMenu();
        if (!opened) {
          console.warn(`  RELAY FALLBACK: could not open Filter by Field for ${relayField}.`);
          continue;
        }

        await page.waitForTimeout(200);
        const clicked = await clickVisibleFieldOption(relayField);
        if (!clicked) {
          console.warn(`  RELAY FALLBACK: could not click ${relayField}.`);
          continue;
        }

        await waitForPostback();
        console.log(`  RELAY FALLBACK: ${relayField} selected; scraping result grid.`);
        await scrapeAllGridPages();
      }
    }
...
async function main() {
  ...
  for (
    ...
    for (
      ...
      if (norm(race.raceType) !== 'skiing') {
        console.log(`  NORDIC OVERRIDE: retaining RaceKey ${race.id} from result-page signature/title despite RaceType=${race.raceType || 'unknown'}`);
      }
