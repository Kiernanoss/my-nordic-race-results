...
async function processRace(race, a, stats, browser) {
  ...
  const scrapeAllPagesForCurrentField = async (fieldGender) => {
    let previousSignature = '';
    let pageNo = 1;
    let totalPages = 1;

    while (pageNo <= totalPages && pageNo <= 200) {
      const current = await parseCurrentPage(fieldGender);
      ...
      const currentRows = [];
      for (const table of parseTables(current.html)) {
        const rows = rowsFromTable(table);
        if (rows.length) currentRows.push(...rows);
      }
      const signature = currentRows.slice(0, 3).map(r => `${r.firstName}|${r.lastName}|${r.time}`).join('||');
      console.log(`  GRID: field ${fieldGender || 'default'} page ${pageNo}/${totalPages} -> ${current.count} new rows`);
      ...
      /*
       * IMPORTANT: Do not search the whole document for Page$N and then call
       * __doPostBack yourself. Endurance Promotions can render several
       * postback controls on the same page, including hidden/secondary grids.
       * The older scraper sometimes selected the wrong control, which made
       * page 2 silently return page 1 again.
       *
       * Prefer the actual visible pager control belonging to the individual
       * results grid, and click it like a real browser user. Only fall back to
       * __doPostBack when a real pager control cannot be clicked.
       */
      const clickedPager = await page.evaluate((wanted) => {
        const wantedArg = `Page$${wanted}`;
        const visible = (el) => {
        ...
        const candidates = [...document.querySelectorAll('a, input, button')].filter(visible);
        ...
        const score = (el) => {
          ...
          let n = 0;
          if (text === String(wanted)) n += 20;
          if (hay.includes(wantedArg)) n += 30;
          if (/grdIndividualResults/i.test(hay)) n += 50;
          const parent = el.closest('table, div');
          if (parent && /grdIndividualResults|pager|paging/i.test(parent.id || parent.className || '')) n += 10;
          return n;
        };

        const ranked = candidates
          .map((el, index) => ({ el, index, score: score(el), href: el.getAttribute('href') || '', onclick: el.getAttribute('onclick') || '', text: (el.textContent || el.value || '').trim() }))
          .filter(x => x.score > 0)
        ...
        best.el.setAttribute('data-nrr-pager-target', '1');
        return { index: best.index, href: best.href, onclick: best.onclick, text: best.text, score: best.score };
      }, nextPage);

      if (clickedPager) {
        console.log(`  GRID: clicking live pager for page ${nextPage} (${clickedPager.text || 'control'})`);
        const pager = page.locator('[data-nrr-pager-target="1"]');
        await pager.click();

        // Wait until the grid actually changes. networkidle alone is not
        // reliable for ASP.NET postbacks because some assets keep connections
        // open and some responses arrive without a full navigation.
        const before = signature;
        const deadline = Date.now() + Math.min(a.timeout, 10000);
        let changed = false;
        while (Date.now() < deadline) {
          await page.waitForTimeout(200);
          const htmlNow = await page.content();
          const rowsNow = [];
          for (const table of parseTables(htmlNow)) {
            const rows = rowsFromTable(table);
            if (rows.length) rowsNow.push(...rows);
          }
          const sigNow = rowsNow.slice(0, 3).map(r => `${r.firstName}|${r.lastName}|${r.time}`).join('||');
          if (sigNow && sigNow !== before) {
            changed = true;
            break;
          }
        }
        if (!changed) {
          console.warn(`  GRID: pager click did not change the result grid for page ${nextPage}; stopping this field.`);
          break;
        }
      } else {
        console.warn(`  GRID: could not locate a visible pager control for page ${nextPage}; trying GridView postback fallback.`);
        const fallback = { target: 'ctl00$cphMain$grdIndividualResults', argument: `Page$${nextPage}` };
        await page.evaluate(({ target, argument }) => {
          if (typeof window.__doPostBack === 'function') window.__doPostBack(target, argument);
        }, fallback);
        await waitForPostback();
      }

      // Remove the temporary marker so the next iteration can identify a
      // fresh live control.
      await page.evaluate(() => {
        document.querySelectorAll('[data-nrr-pager-target="1"]').forEach(el => el.removeAttribute('data-nrr-pager-target'));
      });
  ...
  try {
    ...
    const fields = fieldSelect?.options?.filter(o => {
      const t = o.text.toLowerCase();
      return t && !/^select a field$/.test(t) && !/^show all$/.test(t);
    }) || [];
    ...
    if (fields.length) {
      ...
      for (const field of fields) {
        ...
        if (!freshField) continue;
        try {
          await page.locator('select').nth(freshField.index).selectOption(field.value);
          await waitForPostback();
        } catch (e) {
          console.warn(`  FIELD ${field.text}: select failed: ${e.message}`);
          continue;
        }

        const fieldGender = inferGenderFromText(field.text) || (/^g/i.test(field.text) ? 'F' : /^b/i.test(field.text) ? 'M' : null);
        ...
        await scrapeAllPagesForCurrentField(fieldGender);
      }
    } else {
      console.log('  FIELD FILTER: none detected; scraping the default result set');
      await scrapeAllPagesForCurrentField(null);
    }
