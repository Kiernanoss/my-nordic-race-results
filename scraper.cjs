...
async function fetchGridPage(raceUrl, pageNo, firstPageHtml, a) {
  if (pageNo === 1) {
  ...
  // Endurance Promotions is an ASP.NET site. The visible pager is a
  // postback control; a plain query string can return page 1 again.
  // Try the query-string form first because some deployments support it.
  const queryUrl = new URL(raceUrl);
  queryUrl.searchParams.set('ctl00_cphMain_grdIndividualResultsChangePage', `${pageNo}_50`);
  const queryResponse = await fetchText(queryUrl.href, a.timeout);
  if (queryResponse.body) {
    const rows = parseTables(queryResponse.body).flatMap(rowsFromTable);
    const firstRows = parseTables(firstPageHtml).flatMap(rowsFromTable);
    const firstKey = firstRows[0] ? `${norm(firstRows[0].firstName)}|${norm(firstRows[0].lastName)}|${firstRows[0].time}` : '';
    const queryKey = rows[0] ? `${norm(rows[0].firstName)}|${norm(rows[0].lastName)}|${rows[0].time}` : '';
    if (queryKey && queryKey !== firstKey) return { body: queryResponse.body, method: 'GET' };
  }

  // Fall back to a real ASP.NET form postback.
...
async function processRace(race, a, stats) {
  ...
  // The main results grid. We fetch one page at a time.
  // The site uses ASP.NET postbacks for pages 2+, so fetchGridPage handles
  // both the legacy query-string behavior and the real form postback.
  let totalPages = 1;
  let firstPageHtml = '';

  for (let pageNo = 1; pageNo <= 200; pageNo++) {
    const pageResult = await fetchGridPage(
      race.url,
      pageNo,
      firstPageHtml,
      a
    ...
    if (pageNo === 1) firstPageHtml = pageResult.body;

    const pageTables = parseTables(pageResult.body);
    let rowsFound = 0;
    ...
    // Only the first GET page contains the download/child links.
    if (pageNo === 1) {
      for (const link of parseLinks(pageResult.body)) {
        if (/\.(txt|pdf)(\?|$)/i.test(link.href) || /ResultDetails\.aspx/i.test(link.href)) {
          fileLinks.set(link.href, link);
        }
      }

      const pageInfo = strip(pageResult.body).match(/(\d+)\s+items?\s+in\s+(\d+)\s+pages?/i);
      totalPages = Math.min(Number(pageInfo?.[2]) || 1, 200);
      console.log(`  GRID: ${rowsFound} rows on page 1; ${totalPages} total pages`);
    }

    if (pageNo > 1) {
      console.log(`  GRID: page ${pageNo}/${totalPages} -> ${rowsFound} rows`);
    }

    if (pageNo >= totalPages) break;
    if (pageNo > 1 && rowsFound === 0) break;
  }
  ...
  for (const link of fileLinks.values()) {
    ...
    if (/\.txt/i.test(link.href)) {
      ...
      const rows = parseText(response.body);
      if (rows.length) {
    ...
    if (/\.pdf/i.test(link.href)) {
      try {
        ...
        try {
          ...
          const rows = parseText(parsed.text);
