...
async function processRace(race, a, stats) {
  ...
  // The main results grid. We fetch one page at a time so a large race
  // never creates a second in-memory copy of the entire result set.
  let totalPages = 1;

  for (let pageNo = 1; pageNo <= 200; pageNo++) {
    const pageUrl = new URL(race.url);
    ...
    const response = await fetchText(pageUrl.href, a.timeout);
    if (!response.body) break;

    seenPages.add(pageUrl.href);

    const tables = parseTables(response.body);
    let rowsFound = 0;

    for (const table of tables) {
      const rows = rowsFromTable(table);
      if (rows.length) {
        addRows(rows, pageUrl.href);
        rowsFound += rows.length;
      }
    }

    for (const link of parseLinks(response.body)) {
      if (
        /\.(txt|pdf)(\?|$)/i.test(link.href) ||
        /ResultDetails\.aspx/i.test(link.href)
      ) {
    ...
    if (pageNo === 1) {
      const pageInfo = strip(response.body).match(
