...
async function processRace(race, a, stats, browser) {
   ...
   */
  const buildGridPageUrl = (pageNo, pageSize = 50) => {
    const url = new URL(race.url);
  ...
  /*
   * Scrape every page of the main Individual Results GridView.
   *
   * The previous implementation tried to advance pages with the live
   * ASP.NET __doPostBack()/pager control. That worked on some races, but on
   * others the server returned the same result page even when multiple
   * pages were reported.
   *
   * Endurance Promotions exposes the Individual Results page number in the
   * query string, so request each page explicitly instead of depending on
   * the pager's postback state.
   */
  const scrapeAllGridPages = async () => {
    ...
    const parseMainGrid = async (requestedPage) => {
      const html = await page.content();
      const tables = parseTables(html)
    ...
    const readTotalPages = (html) => {
      const text = clean(strip(html));
      const m = text.match(/(\d+)\s+items?\s+in\s+(\d+)\s+pages?/i);
      return Math.max(1, Number(m?.[2]) || 1);
    };

    // Page 1 is explicitly requested at the same 50-row page size used for
    // later pages. It tells us how many pages the site reports for that grid.
    await page.goto(buildGridPageUrl(1, pageSize), {
      waitUntil: 'domcontentloaded',
      timeout: a.timeout
    });
    await page.waitForTimeout(350);

    let current = await parseMainGrid(1);
    totalPages = readTotalPages(current.html);
    pageSignatures.add(current.signature);

    console.log(`  GRID: page 1/${totalPages} -> ${current.count} new rows`);

    for (let pageNo = 2; pageNo <= totalPages && pageNo <= 200; pageNo++) {
      const targetUrl = buildGridPageUrl(pageNo, pageSize);
      console.log(`  GRID: requesting page ${pageNo}/${totalPages}`);

      let loaded = false;
      let lastCurrent = null;

      // Retry once if the site briefly serves a previously loaded result set.
      for (let attempt = 1; attempt <= 2; attempt++) {
        ...
        lastCurrent = await parseMainGrid(pageNo);

        console.log(
          `  GRID: page ${pageNo}/${totalPages} attempt ${attempt} -> ${lastCurrent.count} new rows`
        );
        console.log(`  GRID: requested URL ${targetUrl}`);
        console.log(`  GRID: loaded URL   ${lastCurrent.actualUrl}`);

        const alreadySeen = pageSignatures.has(lastCurrent.signature);

        if (lastCurrent.signature && !alreadySeen) {
          pageSignatures.add(lastCurrent.signature);
          loaded = true;
          break;
        }

        if (!lastCurrent.signature && lastCurrent.rows.length === 0) {
        ...
        console.warn(
          `  GRID: page ${pageNo} returned a result set already seen; retrying`
        );
      }

      if (!loaded) {
        console.error(
          `  GRID: could not obtain a new result set for page ${pageNo}; stopping this race.`
        );
        break;
      }
  ...
  try {
    ...
    // The Individual Results GridView contains the complete result set
    // (BJV/BVAR/GJV/GVAR). Pagination is requested explicitly with the
    // site's page-number URL parameter; no athlete/team/race is hard-coded.
