 ...
 */
function hasNordicResultSignature(html) {
  const text = norm(strip(String(html || "")));
  const required = ["bjv", "bvar", "gjv", "gvar"];
  const allFour = required.every(field => new RegExp(`\\b${field}\\b`, "i").test(text));
...
function looksNordic(r) {
  ...
  if (r.nordicResultSignature === true) {
    return true;
  }

  // Endurance Promotions occasionally labels MEC/IMAC Nordic events with
  // a non-ski Race Type (for example relays). The event title is a stronger
  // source signal for these meets than Race Type alone. Require the meet
  // family plus Nordic-style event wording so unrelated MEC/IMAC events are
  // not pulled into the Nordic dataset.
  const isMecImac = text.includes('mec') || text.includes('imac');
  const isNordicMeetTitle =
    text.includes('nordic') ||
    text.includes('ski') ||
    text.includes('skate') ||
  ...
  if (isMecImac && isNordicMeetTitle) {
    return true;
  }
      ...
      await mapLimit(
        ...
        async id => {
          ...
          return {
            ...race,
            url,
            nordicResultSignature: hasNordicResultSignature(response.body)
          };
        }
      );
...
async function processRace(race, a, stats, browser) {
  ...
  const findFieldSelect = async () => {
    const selects = await page.evaluate(() => {
      ...
      return all.map((el, index) => {
        ...
        const own = `${el.id || ''} ${el.getAttribute('name') || ''}`.toLowerCase();
        const parentText = (el.parentElement?.innerText || '').slice(0, 500).toLowerCase();
        let score = 0;
        if (/field/.test(own)) score += 8;
        if (/filter\s+by\s+field/.test(parentText)) score += 10;
        const texts = options.map(o => String(o.text || '').trim().toLowerCase());
        if (texts.some(t => ['bjv','bvar','gjv','gvar'].includes(t))) score += 12;
        if (texts.some(t => /middle\s*school|6th/.test(t))) score += 4;
        if (texts.some(t => t === 'select a field')) score += 4;
        return { index, id: el.id || '', name: el.getAttribute('name') || '', options, score };
  ...
  try {
    const initialFieldSelect = await findFieldSelect();

    if (initialFieldSelect) {
      const fields = initialFieldSelect.options
        .map(o => ({ value: o.value, text: o.text.trim() }))
        .filter(o => o.text && !/^select\s+a\s+field$/i.test(o.text))
        .filter(o => /^(bjv|bvar|gjv|gvar)$/i.test(o.text) || /middle\s*school|6th/i.test(o.text));

      if (fields.length) {
        console.log(`  FIELD FILTER: scraping ${fields.length} source fields individually: ${fields.map(f => f.text).join(', ')}`);
        for (const field of fields) {
          await page.goto(race.url, { waitUntil: 'domcontentloaded', timeout: a.timeout });
          await page.waitForTimeout(350);
          const fieldSelect = await findFieldSelect();
          if (!fieldSelect) break;
          try {
            await page.locator('select').nth(fieldSelect.index).selectOption({ value: field.value });
          } catch {
            await page.locator('select').nth(fieldSelect.index).selectOption({ label: field.text });
          }
          await waitForPostback();
          ...
          console.log(`  FIELD FILTER: ${field.text}`);
          await scrapeAllGridPages();
        }
      } else {
        console.log('  FIELD FILTER: selector found but no Nordic-relevant field options; scraping complete result grid.');
        await scrapeAllGridPages();
      }
    } else {
      await page.goto(race.url, { waitUntil: 'domcontentloaded', timeout: a.timeout });
    ...
    // Some MEC/IMAC relay pages expose useful rows only after a field filter is
    // selected, even though the default combined grid is empty. Retry only
    // zero-row races so normal MEC pagination remains unchanged.
    if (all.length === 0) {
      const fallback = await findFieldSelect();
      if (fallback) {
        const fields = fallback.options
          ...
          .filter(o => /^(bjv|bvar|gjv|gvar)$/i.test(o.text) || /middle\s*school|6th/i.test(o.text));
        if (fields.length) {
          console.log(`  FIELD FALLBACK: combined grid returned 0 rows; retrying ${fields.length} fields individually.`);
          for (const field of fields) {
            await page.goto(race.url, { waitUntil: 'domcontentloaded', timeout: a.timeout });
            await page.waitForTimeout(350);
            const fs = await findFieldSelect();
            if (!fs) break;
            try {
              await page.locator('select').nth(fs.index).selectOption({ value: field.value });
            } catch {
              await page.locator('select').nth(fs.index).selectOption({ label: field.text });
            }
            await waitForPostback();
            console.log(`  FIELD FALLBACK: ${field.text}`);
            await scrapeAllGridPages();
          }
        }
      }
