...
function looksNordic(r) {
  // Force the Jan. 13, 2026 MEC Classic Varsity Relays page into scraping.
  // Everything else keeps the original classifier unchanged.
  if (String(r.id) === '1725') return true;
...
async function processRace(race, a, stats, browser) {
  ...
  try {
    ...
    // RaceKey 1725 is a MEC relay page that can load its field selector
    // asynchronously and may show an empty default grid. Do not change the
    // scraping behavior for any other race. For 1725 only, give the selector
    // time to appear and scrape each Nordic field if it exists.
    if (String(race.id) === '1725' && all.length === 0) {
      console.log('  RACE 1725 FALLBACK: waiting for the relay field selector.');
      try {
        await page.waitForTimeout(2000);
        const fieldSelect = await page.locator('select').evaluateAll(selects => selects.map((el, index) => ({
          index,
          options: [...el.options].map(o => ({ value: o.value, text: (o.textContent || '').trim() }))
        })).find(s => {
          const t = s.options.map(o => o.text.toLowerCase());
          return t.some(x => x === 'bjv') && t.some(x => x === 'bvar') && t.some(x => x === 'gjv') && t.some(x => x === 'gvar');
        }) || null);

        if (fieldSelect) {
          console.log('  RACE 1725 FALLBACK: field selector found; scraping BJV/BVAR/GJV/GVAR.');
          for (const opt of fieldSelect.options.filter(o => /^(BJV|BVAR|GJV|GVAR)$/i.test(o.text))) {
            await page.goto(race.url, { waitUntil: 'domcontentloaded', timeout: a.timeout });
            await page.waitForTimeout(2000);
            const selects = page.locator('select');
            try {
              await selects.nth(fieldSelect.index).selectOption({ value: opt.value });
              await waitForPostback();
              console.log(`  RACE 1725 FALLBACK: ${opt.text}`);
              await scrapeAllGridPages();
            } catch (e) {
              console.warn(`  RACE 1725 FALLBACK: ${opt.text} failed: ${e.message}`);
            }
          }
        } else {
          console.warn('  RACE 1725 FALLBACK: selector still not present.');
        }
      } catch (e) {
        console.warn(`  RACE 1725 FALLBACK ERROR: ${e.message}`);
      }
    }
