...
function seasonFor(isoDate) {
  ...
  // Nordic seasons cross calendar years. July–December starts the season;
  // January–June finishes the season that began the previous year.
  ...
  matches.forEach((m) => {
    ...
    if (!byRace.has(raceKey)) {
      ...
      byRace.set(raceKey, {
        ...
        date, season: m.season || seasonFor(date),
...
export default function App() {
  ...
  // Roster seasons come from the team's full dataset, not only the logged-in
  // athlete's seasons. That keeps past team rosters available even if the
  // current athlete did not race in one of those seasons.
  const teamSeasons = useMemo(() => {
    ...
    combined.races.forEach((race) => {
      (race.results || []).forEach((result) => {
        const team = normalizeIdentity(result.team);
        const school = normalizeIdentity(result.school);
        if (team === wanted || school === wanted) values.add(race.season);
      });
    });
    ...
    <div className="nrr-root min-h-screen flex">
      ...
      <main className="flex-1 min-w-0 pb-20 sm:pb-0">
        ...
        <div className="max-w-5xl mx-auto p-5 sm:p-8">
          ...
          {page === "roster" && <RosterPage profile={appState.profile} teamMembers={teamMembers} myAthleteId={myAthleteId} appState={appState} setAppState={setAppState} goto={goto} seasons={teamSeasons} />}
...
function SeasonsPage({ myResults, seasons, goto }) {
  ...
  const rows = myResults.filter((r) => r.race.season === selectedSeason);
  const positions = rows.map((r) => primaryPlace(r.result)).filter(Number.isFinite);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <select value={selectedSeason} onChange={(e) => setSelectedSeason(e.target.value)} className="nrr-focus text-sm border rounded-full px-3 py-2" style={{ borderColor: "var(--border)" }} aria-label="Select season">
          {seasons.map((season) => <option key={season} value={season}>{season}</option>)}
        </select>
      </div>
      {!selectedSeason || rows.length === 0 ? (
        <Card className="p-6"><p className="text-sm" style={{ color: "var(--slate)" }}>No races for this season yet.</p></Card>
      ) : (
        <Card className="p-6">
          <h3 className="nrr-display text-xl mb-4">{selectedSeason}</h3>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-4">
            <StatCard label="Races" value={rows.length} />
            <StatCard label="Best Finish" value={positions.length ? placeSuffix(Math.min(...positions)) : "Not available"} />
            <StatCard label="Avg Finish" value={positions.length ? (positions.reduce((a, b) => a + b, 0) / positions.length).toFixed(1) : "Not available"} />
            <StatCard label="Best 5K" value={rows.filter((r) => r.result.distanceKm === 5).sort((a, b) => a.result.totalTimeSec - b.result.totalTimeSec)[0]?.result.totalTime || "Not available"} />
            <StatCard label="Best 2.5K" value={rows.filter((r) => r.result.distanceKm === 2.5).sort((a, b) => a.result.totalTimeSec - b.result.totalTimeSec)[0]?.result.totalTime || "Not available"} />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
            <StatCard label="Podiums" value={podiums} />
            <StatCard label="Best Time" value={bestTime} />
            <StatCard label="Races Completed" value={rows.length} />
          </div>
          <div className="flex flex-col divide-y" style={{ borderColor: "var(--border-soft)" }}>
            {rows.map(({ race, result }) => (
...
/* ============================================================================
   ROSTER PAGE
============================================================================ */
function RosterPage({ profile, teamMembers, myAthleteId, appState, setAppState, goto, seasons }) {
  const [rowsByAthlete, setRowsByAthlete] = useState({});
  ...
  useEffect(() => {
    if (latestSeason && !seasons.includes(selectedSeason)) setSelectedSeason(latestSeason);
  }, [latestSeason, seasons, selectedSeason]);

  // An athlete is in a season's roster only after they have raced in that season.
  // If they raced both JV and varsity during the selected season, use the level
  // from their most recent race in that same season.
  const athleteRows = teamMembers.map((a) => {
    const results = (rowsByAthlete[a.id] || [])
      .filter((r) => r.race.season === selectedSeason)
      .slice()
      .sort((a, b) => {
        const dateDiff = new Date(b.race.date) - new Date(a.race.date);
        if (dateDiff !== 0) return dateDiff;
        return String(b.race.id || "").localeCompare(String(a.race.id || ""));
      });
  ...
  const rosterGroups = ROSTER_CLASS_ORDER.map((classKey) => {
    const rows = athleteRows.filter((row) => row.latest && row.latestClass === classKey);
    rows.sort((a, b) => {
      const placeA = primaryPlace(a.latest.result);
      const placeB = primaryPlace(b.latest.result);
      const placeDiff = (Number.isFinite(placeA) ? placeA : 999999) - (Number.isFinite(placeB) ? placeB : 999999);
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm" style={{ color: "var(--slate)" }}>Most recent race in the selected season, then team finish from first to last.</p>
        </div>
        <select value={selectedSeason} onChange={(e) => setSelectedSeason(e.target.value)} className="nrr-focus text-sm border rounded-full px-3 py-2" style={{ borderColor: "var(--border)" }} aria-label="Select roster season">
          {seasons.map((season) => <option key={season} value={season}>{season}</option>)}
        </select>
      </div>
      {!selectedSeason ? (
        <Card className="p-6"><p className="text-sm" style={{ color: "var(--slate)" }}>No season data is available yet.</p></Card>
      ) : rosterGroups.length === 0 ? (
        <Card className="p-6"><p className="text-sm" style={{ color: "var(--slate)" }}>No team members have raced in {selectedSeason} yet.</p></Card>
      ) : (
...
function PerformancePage({ myResults, goto }) {
  ...
  const latestSeason = seasons[seasons.length - 1] || "";
  const [seasonFilter, setSeasonFilter] = useState(latestSeason);

  useEffect(() => {
    if (latestSeason && !seasons.includes(seasonFilter)) setSeasonFilter(latestSeason);
  }, [latestSeason, seasons, seasonFilter]);

  if (myResults.length === 0) return <div className="text-sm" style={{ color: "var(--slate)" }}>Not enough results yet to chart performance.</div>;

  const filteredResults = seasonFilter === "All Time"
    ? myResults
    : myResults.filter((r) => r.race.season === seasonFilter);
  const chartData = [...filteredResults].sort((a, b) => a.race.date.localeCompare(b.race.date)).map((r, i) => ({
    i: i + 1,
    place: primaryPlace(r.result),
    pace: pacePerKm(r.result.totalTimeSec, r.result.distanceKm),
    distance: r.result.distanceKm,
    name: r.race.raceName,
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <select value={seasonFilter} onChange={(e) => setSeasonFilter(e.target.value)} className="nrr-focus text-sm border rounded-full px-3 py-2" style={{ borderColor: "var(--border)" }} aria-label="Select performance season">
          <option value="All Time">All Time</option>
          {seasons.map((season) => <option key={season} value={season}>{season}</option>)}
        </select>
      </div>
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4"><h3 className="font-medium">Finish Position Over Time</h3><span className="text-xs" style={{ color: "var(--slate)" }}>{seasonFilter === "All Time" ? "All seasons" : seasonFilter} · Lower is better</span></div>
        <ResponsiveContainer width="100%" height={240}>
