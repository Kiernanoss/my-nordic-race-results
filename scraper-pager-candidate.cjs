...
async function processRace(race, a, stats, browser) {
  ...
  const scrapeAllGridPages = async () => {
    ...
    const getMainGridSignature = async () => {
      return page.evaluate(() => {
        ...
        const candidates = tables
          .map(t => {
            ...
            return {
              rows: rows.length,
              text: rows.slice(1, 8).map(r => (r.innerText || '').replace(/\s+/g, ' ').trim()).join('||')
            };
          })
          .filter(x => x.rows > 1 && x.text)
          .sort((a, b) => b.rows - a.rows);
        return candidates[0]?.text || '';
      });
    };

    const findPagerControls = async (wantedPage) => {
      return page.evaluate((wanted) => {
        ...
        const addPostback = (post, outerHTML, source) => {
          if (!post || !post.argument.endsWith(`Page$${wanted}`)) return;
          const key = `postback|${post.target}|${post.argument}`;
          ...
          controls.push({
            ...
            priority: /grdIndividualResults/i.test(post.target) ? 100 : 0
          });
        };
        ...
        for (const el of elements) {
          ...
          for (const name of ['href', 'onclick', 'oncommand', 'data-href']) {
            addPostback(parsePostback(attrValue(el, name)), el.outerHTML, name);
          }

          if (el.tagName === 'A' || el.tagName === 'INPUT' || el.tagName === 'BUTTON') {
            const key = `click|${el.outerHTML.slice(0, 500)}`;
            if (!seen.has(key)) {
              seen.add(key);
              controls.push({
                mode: 'click',
                ...
                priority: /grdIndividualResults/i.test(el.outerHTML) ? 80 : 0
              });
            }
          }
        }

        // Last fallback: collect every Page$N postback on the page, even when
        // the visible control text is not exactly the requested page number.
        for (const el of elements) {
          for (const name of ['href', 'onclick', 'oncommand']) {
            addPostback(parsePostback(attrValue(el, name)), el.outerHTML, `fallback-${name}`);
          }
        }

        controls.sort((a, b) => b.priority - a.priority);
        return controls;
      }, wantedPage);
    };
    ...
    const triggerNextPage = async (nextPage) => {
      ...
      for (let i = 0; i < controls.length; i++) {
        ...
        try {
          ...
          console.warn(`  GRID: pager candidate ${i + 1} did not change the main result grid; trying the next candidate.`);
        } catch (err) {
          console.warn(`  GRID: pager candidate ${i + 1} failed: ${err.message}`);
        }
      }

      console.warn(`  GRID: none of the page ${nextPage} pager candidates changed the main result grid.`);
      return false;
    };
